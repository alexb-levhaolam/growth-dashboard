...
const PROJ_ST={done:{l:'готово',bg:'#6E9B0E',tx:'#fff'},progress:{l:'в работе',bg:'#F18B0E',tx:'#fff'},test:{l:'тест',bg:'#1761CB',tx:'#fff'},risk:{l:'риск',bg:'#DD2A02',tx:'#fff'},wait:{l:'ожидание',bg:'#585E65',tx:'#fff'},blocked:{l:'блокер',bg:'#DD2A02',tx:'#fff'}}
...
function Label({children}){return<p style={{fontSize:15,color:S.i3,margin:'0 0 12px 2px',fontWeight:600}}>{children}</p>}
...
function Ed({value,onSave,canEdit,multi,ph,style={}}){const[editing,setEditing]=useState(false);const[val,setVal]=useState(value);useEffect(()=>setVal(value),[value]);if(!canEdit)return<Linkify style={style}>{value||''}</Linkify>;if(!editing)return<span style={{...style,cursor:'pointer',borderBottom:'1px dashed #C4C8CD'}} onClick={()=>setEditing(true)}>{value?<Linkify style={style}>{value}</Linkify>:<span style={style}>{ph||'(кликни)'}</span>}</span>;const save=()=>{setEditing(false);if(val!==value)onSave(val)};if(multi)return<textarea value={val||''} onChange={e=>setVal(e.target.value)} onBlur={save} autoFocus rows={2} style={{width:'100%',padding:'6px 10px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:17,resize:'vertical',outline:'none',fontFamily:'inherit',fontSize:17,...style}}/>;return<input value={val||''} onChange={e=>setVal(e.target.value)} onBlur={save} onKeyDown={e=>e.key==='Enter'&&save()} autoFocus style={{width:'100%',padding:'4px 8px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:'inherit',outline:'none',...style}}/>}
...
function Main({profile}){
  ...
  return<div style={{minHeight:'100vh',background:S.bg}}>
    ...
    <div style={{background:'linear-gradient(135deg,#0F6E5E,#0B5548)',borderRadius:14,padding:'32px 32px',marginBottom:32}} className="no-print">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <button onClick={()=>supabase.auth.signOut()} style={{fontSize:14,fontWeight:600,color:'#FFFFFF',background:'rgba(255,255,255,.16)',border:'1px solid rgba(255,255,255,.4)',border:'none',borderRadius:10,padding:'4px 10px',cursor:'pointer'}}>Выйти</button>
        </div>
      </div>
      {rep&&<><h1 style={{margin:'4px 0 0',fontSize:32,color:'#fff',fontWeight:700}}>Неделя <Ed value={rep.week_label} canEdit={ce} onSave={v=>upRep({week_label:v})} style={{fontSize:32,color:'#fff',fontWeight:700}}/></h1>
        ...
        <div style={{marginTop:10,display:'flex',gap:4,flexWrap:'wrap',alignItems:'center'}}>
          ...
          {isA&&rep&&<button onClick={deleteWeek} style={{padding:'6px 14px',borderRadius:8,border:'none',cursor:'pointer',fontSize:14,fontWeight:600,background:'rgba(255,99,63,.25)',color:'#FF8A6F',display:'flex',alignItems:'center',gap:6}} title="Удалить текущий отчёт"><i className="lucide lucide-trash-2" style={{fontSize:15}}/>Удалить отчёт</button>}
        </div>
    </div>
    <div style={{display:'flex',gap:2,marginBottom:16,background:S.sf,borderRadius:10,padding:3,border:`1px solid ${S.ln}`,overflowX:'auto',WebkitOverflowScrolling:'touch'}} className="no-print">
      {tabs.map(t=><button key={t.id} onClick={()=>setView(t.id)} style={{flex:'0 0 auto',padding:'8px 14px',border:'none',borderRadius:8,cursor:'pointer',fontSize:15,fontWeight:600,background:view===t.id?S.gd:'transparent',color:view===t.id?'#fff':S.i2,whiteSpace:'nowrap'}}>{t.l}</button>)}
    </div>
  ...
  return<div id="overview-print"><>
    ...
    {showPaste&&<div style={{background:S.sf,border:`1px solid ${S.gl}`,borderRadius:14,padding:16,marginBottom:28}}>
      ...
      <div style={{display:'flex',gap:8,marginTop:8}}>
        <button onClick={applyPaste} style={{padding:'8px 20px',borderRadius:8,border:'none',background:S.gm,color:'#fff',fontSize:17,fontWeight:600,cursor:'pointer'}}>Применить</button>
    ...
    return<div style={{background:'linear-gradient(135deg,#0F6E5E,#0B5548)',borderRadius:14,padding:'14px 20px',marginBottom:16,color:S.gp}}>
    ...
    <div style={{background:'linear-gradient(135deg,#0F6E5E,#0B5548)',borderRadius:14,padding:'22px 16px'}}><div style={{fontSize:17,fontWeight:600,color:S.gs,marginBottom:6}}>Нужно от руководства</div><div style={{color:S.gp}}><EList items={rep.asks} canEdit={ce} onSave={v=>up({asks:v})}/></div></div>
  </></div>
}
...
function Projects({projects,setProjects,comments,setComments,ce,reports,aIdx,profile,reload}){
  ...
  return<>
    ...
    {showNewP&&<div style={{background:S.sf,border:`1px solid ${S.gl}`,borderRadius:14,padding:16,marginBottom:12}}>
      ...
      <button onClick={addProj} disabled={!np.id.trim()||!np.name.trim()} style={{padding:'10px 18px',borderRadius:8,border:'none',background:S.gm,color:'#fff',fontSize:15,fontWeight:600,cursor:'pointer'}}>Создать</button>
    </div>}
