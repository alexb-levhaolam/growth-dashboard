...
function Box({title,children}){return<div style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:'24px',marginBottom:22}}><div style={{fontSize:17,fontWeight:600,marginBottom:14}}>{title}</div>{children}</div>}
function CC({title,children,right}){return<div style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:'24px',marginBottom:22}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}><div style={{fontSize:17,fontWeight:600}}>{title}</div>{right&&<div>{right}</div>}</div>{children}</div>}
...
function Ed({value,onSave,canEdit,multi,ph,style={}}){const[editing,setEditing]=useState(false);const[val,setVal]=useState(value);useEffect(()=>setVal(value),[value]);if(!canEdit)return<Linkify style={{whiteSpace:'pre-wrap',...style}}>{value||''}</Linkify>;if(!editing)return<span style={{...style,cursor:'pointer',borderBottom:'1px dashed #C4C8CD',whiteSpace:'pre-wrap',display:'inline-block'}} onClick={()=>setEditing(true)}>{value?<Linkify style={{whiteSpace:'pre-wrap',...style}}>{value}</Linkify>:<span style={{...style,color:'#9AA0A6'}}>{ph||'(кликни)'}</span>}</span>;const save=()=>{setEditing(false);if(val!==value)onSave(val)};const cancel=()=>{setEditing(false);setVal(value)};return<div style={{width:'100%'}}><textarea value={val||''} onChange={e=>setVal(e.target.value)} autoFocus rows={Math.max(2,String(val||'').split('\n').length+1)} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:`1px solid ${S.gd}`,fontSize:14,resize:'vertical',outline:'none',fontFamily:'inherit',color:'#121416',background:'#fff',lineHeight:1.5,boxSizing:'border-box'}}/><div style={{display:'flex',gap:6,marginTop:4}}><button onClick={save} style={{padding:'4px 14px',borderRadius:6,border:'none',background:S.gd,color:'#fff',fontSize:12,fontWeight:600,cursor:'pointer'}}>Сохранить</button><button onClick={cancel} style={{padding:'4px 10px',borderRadius:6,border:`1px solid ${S.ln}`,background:'#fff',color:S.i3,fontSize:12,cursor:'pointer'}}>Отмена</button></div></div>}
...
function Main({profile}){
  ...
  const navTo=(p)=>{setPage(p);setMenuOpen(false)};return<div className='lh-app' style={{display:'grid',gridTemplateColumns:'220px 1fr',minHeight:'100vh',background:S.bg}}>
    ...
    {/* HOME */}
    {page==='home'&&<>
      <h1 style={{fontSize:24,fontWeight:600,marginBottom:22}}>Добро пожаловать{profile.name?', '+profile.name:''}</h1>
    ...
    {page==='weekly'&&rep&&<>
      ...
      {view==='tactical'&&<Tactical tasks={tTasks} progress={tProgress} reports={reports} aIdx={aIdx} ce={ce} reload={reload} profile={profile} mPlans={mPlans}/>}
    ...
    {page==='integrations'&&isA&&<>
      <h1 style={{fontSize:24,fontWeight:600,marginBottom:22}}>Интеграции</h1>
...
function Overview({rep,reports,projects,comments,ce,up,tTasks,tProgress,print,refreshDaily,mPlans}){
  ...
  const wow=(cur,prev)=>{if(cur==null||prev==null||prev===0)return null;return Math.round((cur-prev)/prev*100)}
  const WoW=({cur,prev,invert})=>{const d=wow(cur,prev);if(d==null)return null;const good=invert?(d<0):(d>0);return<span style={{fontSize:12,fontWeight:600,color:good?'#497B02':'#A71F00',marginLeft:6}}>{d>0?'+':''}{d}% vs пр.нед</span>}
  ...
  return<div id="overview-print"><>
    ...
    <div className='lh-grid4' style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16,marginBottom:28}}>
      {[
      ...
      ].map((x,i)=><div key={i} style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:'18px 16px'}}>
        <div style={{fontSize:14,color:S.i3,display:'flex',alignItems:'center'}}>{x.l}<Info text={x.info}/></div>
        <div className='lh-num' style={{fontSize:28,fontWeight:600,margin:'4px 0'}}>
          {x.k?<>{x.pre||''}<EdNum value={m[x.k]} canEdit={ce} onSave={v=>upM(x.k,v)} style={{fontSize:28,fontWeight:600}}/></>
          :x.k2?<>{x.pre||''}<EdNum value={x.v2} canEdit={ce} onSave={v=>{if(v==null)upM(x.k2,null);else upM(x.k2,v)}} style={{fontSize:28,fontWeight:600}}/></>
          :x.v}
        </div>
    ...
    <div className='lh-grid7' style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:8,marginBottom:16}}>
      {(daily.length>0?daily:Array(7).fill(null)).map((d,i)=>{const chTip=d?.ch?Object.entries(d.ch).filter(([,v])=>v>0).map(([k,v])=>`${k}: ${v}`).join('\n'):'';return<div key={i} title={chTip} style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:'12px 8px',textAlign:'center',cursor:chTip?'help':'default'}}>
    ...
    <div className='lh-tasks lh-grid3' style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:32}}>
      {visibleProjects.map(p=>{const ps=PROJ_ST[p.status]||PROJ_ST.wait;const lc=comments.find(c=>c.project_id===p.id&&c.week_start===rep.week_start);const txt=lc?(lc.full_text||lc.summary||''):'';const isLong=txt.length>150;const isFull=expandedComm[p.id]
    ...
    <div className='lh-analysis' style={{display:'flex',gap:12,marginBottom:16,flexWrap:'wrap'}}>
      <div style={{flex:1,background:S.sf,borderRadius:14,boxShadow:S.sh,padding:'22px 16px'}}><div style={{fontSize:17,fontWeight:600,color:'#3B6D11',marginBottom:6}}> Инсайты и наблюдения</div><EList items={rep.manual_improved} canEdit={ce} onSave={v=>up({manual_improved:v})} color="#27500A"/></div>
...
// ═══ TACTICAL TASKS ═══
function Tactical({tasks,progress,reports,aIdx,ce,reload,profile,mPlans}){
...
function Projects({projects,setProjects,comments,setComments,ce,reports,aIdx,profile,reload,rep:currentRep,upRep}){
  ...
  const ProjCard=({p})=>{const ps=PROJ_ST[p.status]||PROJ_ST.wait;const wcs=getWC(p.id);const prev=getPrev(p.id);const isO=exp===p.id;const isBlocked=p.status==='blocked';const hasBlock=p.constraints_text&&p.constraints_text.trim();const ds=dates(p);const lastC=wcs[0]||prev
    return<div style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:'16px 20px',marginBottom:12,borderLeft:isBlocked?'3px solid #DD2A02':'none',borderRadius:isBlocked?'0 14px 14px 0':14}}>
      ...
      {/* Показывать в обзоре */}
      {isO&&ce&&currentRep&&<label onClick={e=>e.stopPropagation()} style={{display:'flex',alignItems:'center',gap:6,marginTop:10,cursor:'pointer',fontSize:13,color:S.i3,paddingTop:8,borderTop:`1px solid ${S.ln}`}}><input type="checkbox" checked={(currentRep.visible_overview_projects||[]).includes(p.id)} onChange={()=>{const vis=currentRep.visible_overview_projects||[];const next=vis.includes(p.id)?vis.filter(id=>id!==p.id):[...vis,p.id];upRep({visible_overview_projects:next})}}/> показывать в обзоре</label>}
...
function Admin({profiles,projects,reports,aIdx,reload,rep,upRep}){const[pins,setPins]=useState(rep?.pinned_projects||[]);useEffect(()=>setPins(rep?.pinned_projects||[]),[rep]);const upRole=async(id,r)=>{await supabase.from('profiles').update({role:r}).eq('id',id);reload()};const upPP=async(id,p)=>{await supabase.from('projects').update({priority:p}).eq('id',id);reload()};const togPin=async pid=>{const n=pins.includes(pid)?pins.filter(x=>x!==pid):[...pins,pid];setPins(n);if(rep)await supabase.from('weekly_reports').update({pinned_projects:n}).eq('id',rep.id);reload()}
  ...
  return<><Box title={`Каналы в Обзоре · ${rep?.week_label||''}`}><div style={{fontSize:15,color:S.i3,marginBottom:12}}>Включи/выключи каналы для отображения на этой неделе.</div>{allChNames.map(name=>{const ch=(rep?.channels||[]).find(c=>c.name===name);const on=visCh.includes(name);return<div key={name} style={{display:'flex',alignItems:'center',gap:10,padding:'6px 0',borderBottom:`1px solid ${S.ln}`,fontSize:17}}><input type="checkbox" checked={on} onChange={()=>togCh(name)} style={{cursor:'pointer'}}/><span style={{flex:1,fontWeight:500,opacity:on?1:0.4}}>{name}</span><span style={{fontSize:15,color:S.i3}}>{ch?.sales||0} продаж</span>{ch?.cpo&&<span style={{fontSize:15,color:S.i2}}>CPO ${ch.cpo}</span>}</div>})}</Box>
    <Box title="Пользователи">{profiles.map(p=><div key={p.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:`1px solid ${S.ln}`}}><div><span style={{fontWeight:500}}>{p.name||p.email}</span><span style={{fontSize:15,color:S.i3,marginLeft:8}}>{p.email}</span></div><select value={p.role} onChange={e=>upRole(p.id,e.target.value)} style={{padding:'7px 10px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:17}}><option value="admin">admin</option><option value="editor">editor</option><option value="viewer">viewer</option></select></div>)}</Box>
...
function MonthlyReport({reports,projects,comments,mPlans,setMPlans,ce,reload}){
  ...
  if(month==null)return<>
    ...
    <div style={{display:'flex',gap:8,marginBottom:22}}><button onClick={()=>setYear(2026)} style={{padding:'10px 18px',borderRadius:8,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:S.gd,color:'#fff',boxShadow:S.sh}}>2026</button></div>
  ...
  return<>
    ...
    {/* ═══ 1. STATUS ═══ */}
    <div className='lh-mplate' style={{background:(localStatus||plan?.status)==='red'?'linear-gradient(180deg,#DD2A02,#A71F00)':(localStatus||plan?.status)==='green'?'linear-gradient(180deg,#6E9B0E,#497B02)':'linear-gradient(180deg,#F18B0E,#B76400)',borderRadius:14,padding:32,color:'#fff',marginBottom:22}}>
    ...
    return<div className='lh-mteal' style={{background:'linear-gradient(180deg,#0F6E5E,#0B5548)',borderRadius:14,padding:'28px 32px',color:'#fff',marginBottom:22}}>
      ...
      <div className='lh-grid4' style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:24}}>
        {[{l:'Продажи',f:plan?.is_closed?plan.closed_sales:sel.totalSales,p:plan?.total_plan,pk:'total_plan',sp:wSales,inv:false,cf:mcData?.totalSales},{l:'CPO Ads',f:plan?.override_cpo_ads!=null?plan.override_cpo_ads:(plan?.is_closed?plan.closed_cpo_ads:cpoAds),p:plan?.plan_cpo_ads,pk:'plan_cpo_ads',ok:'override_cpo_ads',sp:wCpoAds,inv:true,pre:'$',cf:mcCpoAds},{l:'CPO Total',f:plan?.override_cpo_total!=null?plan.override_cpo_total:(plan?.is_closed?plan.closed_cpo_total:cpoTotal),p:plan?.plan_cpo_total,pk:'plan_cpo_total',ok:'override_cpo_total',sp:wCpoTotal,inv:true,pre:'$',cf:mcCpoTotal},{l:'Бюджет',f:plan?.override_budget!=null?plan.override_budget:(plan?.is_closed?plan.closed_budget:(sel.totalSpentBM||0)),p:plan?.ads_budget,pk:'ads_budget',ok:'override_budget',sp:ws.map(w=>w.metrics?.totalSpentBM||0),inv:true,pre:'$',cf:mcData?.totalSpentBM}].map((x,i)=>{const d=pctD(x.f,x.p);const isGood=x.inv?(d&&d<0):(d&&d>0);const cd=x.cf!=null&&x.f!=null&&x.cf!==0?Math.round((x.f-x.cf)/x.cf*100):null;const cGood=x.inv?(cd&&cd<0):(cd&&cd>0)
          return<div key={i}>
          <div style={{fontSize:14,color:'rgba(255,255,255,.6)',marginBottom:8}}>{x.l}</div>
          <div style={{display:'flex',alignItems:'baseline',gap:10}}><span className='lh-num' style={{fontSize:36,fontWeight:600}}>{x.pre||''}<EdNum value={x.f} canEdit={ce} onSave={v=>savePlan(x.ok,v)} style={{fontSize:36,fontWeight:600,color:'#fff'}}/></span><Spark data={x.sp} color={d==null?'rgba(255,255,255,.4)':isGood?'#AAD34F':'#FF8A6F'}/></div>
    ...
    {(()=>{const churnFields=[{k:'churn_base',l:'Активная база',isBs:true},{k:'churn_total',l:'Общий отток'},{k:'churn_0_12',l:'Отток в шт.',isBs:true},{k:'churn_0_3',l:'Отток 0–3 мес'},{k:'churn_4_6',l:'Отток 4–6 мес'},{k:'churn_7_12',l:'Отток 7–12 мес'},{k:'churn_13_24',l:'Отток 13–24 мес'},{k:'churn_25p',l:'Отток 25+ мес'}];const cc=churnCmp!=null?churnCmp:(month>0?month-1:11);const cmpKey=`${year}-${String(cc+1).padStart(2,'0')}`;const cmpPlan=mPlans.find(p=>p.id===cmpKey)
      return<div style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:22,marginBottom:22}}>
    ...
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
      <span style={{fontSize:15,fontWeight:600,color:S.i3,letterSpacing:'.06em',textTransform:'uppercase'}}>Каналы</span>
      {ce&&<select onChange={e=>{if(!e.target.value)return;const hc=plan?.hidden_channels||[];const next=hc.includes(e.target.value)?hc:([...hc,e.target.value]);saveText('hidden_channels',next);e.target.value=''}} style={{padding:'4px 8px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:12,cursor:'pointer'}}><option value="">скрыть канал...</option>{(sel.channels||[]).filter(ch2=>!(plan?.hidden_channels||[]).includes(ch2.name)).map(ch2=><option key={ch2.name} value={ch2.name}>{ch2.name}</option>)}</select>}
    </div>
    {(plan?.hidden_channels||[]).length>0&&ce&&<div style={{marginBottom:8,display:'flex',gap:4,flexWrap:'wrap'}}><span style={{fontSize:12,color:S.i3}}>Скрытые:</span>{(plan?.hidden_channels||[]).map(n=><button key={n} onClick={()=>saveText('hidden_channels',(plan?.hidden_channels||[]).filter(x=>x!==n))} style={{fontSize:11,padding:'2px 8px',borderRadius:6,border:`1px solid ${S.ln}`,background:'#F7F8F9',cursor:'pointer'}}>{n} ↩</button>)}</div>}
    <div style={{background:'#fff',borderRadius:14,boxShadow:S.sh,padding:'6px 12px',marginBottom:22}}>
      ...
      <tbody>{(sel.channels||[]).filter(c=>(c.sales>0||c.spent>0)&&!(plan?.hidden_channels||[]).includes(c.name)).map((c,i)=>{const cp2=(plan?.channel_plans||{})[c.name?.toLowerCase?.()]||{};const ps=cp2.planSales||null;const pct=ps&&c.sales?Math.round(c.sales/ps*100):null;const cpo=c.spent&&c.sales?Math.round(c.spent/c.sales):null;return<tr key={i} style={{borderBottom:'1px solid #E4E6E9'}}>
    ...
    {ce&&hiddenList.length>0&&<div style={{marginBottom:22}}><span style={{fontSize:13,color:S.i3}}>Скрытые ({hiddenList.length}): </span>{hiddenList.map(p=><button key={p.id} onClick={()=>saveText('hidden_projects',hidden.filter(id=>id!==p.id))} style={{fontSize:13,padding:'7px 10px',borderRadius:8,border:`1px solid ${S.ln}`,background:'#F7F8F9',cursor:'pointer',marginRight:4}}>{p.name} ↩</button>)}</div>}
    </>})()}
...
{/* ═══ 7. УЛУЧШИЛОСЬ / УХУДШИЛОСЬ — monthly summary ═══ */}
    ...
    <div style={{background:'#EDF5FF',borderRadius:14,padding:22,marginBottom:22}}><div style={{fontSize:17,fontWeight:600,color:'#144CA5',marginBottom:14}}>План и приоритеты</div><EList items={plan?.next_focus||[]} canEdit={ce} onSave={v=>saveText('next_focus',v)} color="#144CA5"/></div>
...
function YoYReport(){
  ...
  const Chart=({field,prefix,yr1,yr2,color1,color2,height})=>{
    ...
    const onMove=(e,yr)=>{const rect=e.currentTarget.getBoundingClientRect();const x=e.clientX-rect.left;const mi=Math.round((x-pad.l)/((w-pad.l-pad.r)/11));if(mi<0||mi>11)return;const v=get(yr,mi+1,field);if(v!=null){const ann=data.find(d2=>d2.year===yr&&d2.month===mi+1)?.annotation;setTooltip({x:e.clientX,y:e.clientY,text:`${MF[mi]} ${yr}: ${prefix||''}${v.toLocaleString()}${ann?' · '+ann:''}`.slice(0,120)})}}
  ...
  return<>
    ...
    {mode==='compare'&&<>
      ...
      <div style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:'8px 0',marginBottom:24,overflowX:'auto'}}>
        ...
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:14,minWidth:800}}><thead><TH/></thead><tbody>
          <ChRow label="CPO Meta" field="cpo_meta" yr1={2025} yr2={2026} color1="#FF6C1C" color2="#F6360B"/>
          <ChRow label="CPO Google" field="cpo_google" yr1={2025} yr2={2026}/>
        </tbody></table>
      </div>
      ...
      <div style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:'8px 0',marginBottom:24,overflowX:'auto'}}>
        ...
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:14,minWidth:800}}><thead><TH/></thead><tbody>
          <ChRow label="Meta" field="spend_meta" yr1={2025} yr2={2026} color1="#FF6C1C" color2="#F6360B"/>
          <ChRow label="Google" field="spend_google" yr1={2025} yr2={2026}/>
        </tbody></table>
      </div>
      ...
      {/* Annotations */}
      {data.filter(d=>d.annotation).length>0&&<div style={{background:S.sf,borderRadius:14,boxShadow:S.sh,padding:22,marginBottom:22}}>
