...
const S={bg:'#FCFDFD',sf:'#FFFFFF',ink:'#121416',i2:'#585E65',i3:'#737A82',ln:'#C4C8CD',gd:'#6E9B0E',gm:'#497B02',gl:'#91B93D',gs:'#C1E274',gp:'#F7FBEA',ol:'#585E65',am:'#F18B0E',rm:'#DD2A02',bm:'#1761CB',cta:'#FF6C1C',ctaH:'#F05400',sh:'0 4px 20px rgba(18,20,22,.08)'}
...
function Chip({children,bg,tx}){return<span style={{display:'inline-flex',alignItems:'center',height:25,fontSize:13,fontWeight:600,padding:'0 12px',borderRadius:999,background:bg,color:tx,whiteSpace:'nowrap'}}>{children}</span>}
...
function Label({children}){return<p style={{fontSize:17,color:S.i3,margin:'0 0 12px 2px',fontWeight:600}}>{children}</p>}
function Box({title,children}){return<div style={{background:S.sf,borderRadius:16,boxShadow:S.sh,padding:'22px',marginBottom:22}}><div style={{fontSize:20,fontWeight:600,marginBottom:14}}>{title}</div>{children}</div>}
function CC({title,children,right}){return<div style={{background:S.sf,borderRadius:16,boxShadow:S.sh,padding:'22px',marginBottom:22}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}><div style={{fontSize:20,fontWeight:600}}>{title}</div>{right&&<div>{right}</div>}</div>{children}</div>}
  ...
  return<div id="overview-print"><>
    <div style={{display:'flex',gap:8,marginBottom:14,fontSize:15,color:S.i3,flexWrap:'wrap',alignItems:'center'}}>
      ...
      <span style={{marginLeft:'auto',display:'flex',gap:4}}>
        ...
        {ce&&refreshDaily&&<button onClick={refreshDaily} title="Обновить данные на основе LH Daily" style={{fontSize:17,color:S.sf,background:S.gd,border:'none',borderRadius:8,padding:'10px 18px',cursor:'pointer',fontWeight:600}}>Обновить из Daily <i className="lucide lucide-info" style={{fontSize:15,opacity:.7}}/></button>}
    ...
    {showPaste&&<div style={{background:S.sf,border:`1px solid ${S.gl}`,borderRadius:14,padding:16,marginBottom:28}}>
      ...
      <div style={{display:'flex',gap:8,marginTop:8}}>
        <button onClick={applyPaste} style={{padding:'8px 20px',borderRadius:8,border:'none',background:S.cta,color:'#fff',fontSize:17,fontWeight:600,cursor:'pointer'}}>Применить</button>
    ...
    return<div style={{background:'linear-gradient(135deg,#497B02,#234003)',borderRadius:14,padding:'14px 20px',marginBottom:16,color:S.gp}}>
    ...
    <Label>Каналы</Label>
    <div style={{background:S.sf,borderRadius:16,boxShadow:S.sh,overflow:'hidden',marginBottom:28}}>
      <div style={{display:'grid',gridTemplateColumns:'110px 1fr 70px 60px 55px 60px 70px 80px 60px',gap:4,padding:'14px 16px',background:S.bg,fontSize:11,fontFamily:'Montserrat,sans-serif',fontWeight:700,color:S.i3,textTransform:'uppercase',letterSpacing:'.1em'}}>
      ...
      {ch.filter(c=>!visCh||visCh.includes(c.name)).map((c,i)=>{const ci=ch.indexOf(c);const mp=getChPlan(c.name);const ps=c.planSales!=null?c.planSales:(mp.planSales||0);const barPct=ps?(Math.min((c.sales||0)/ps*100,100)):((c.sales||0)>0?100:0);const barColor=ps?((c.sales||0)>=ps?'#497B02':(c.sales||0)>=ps*0.7?'#F18B0E':'#DD2A02'):((c.sales||0)>0?S.gl:'#E4E6E9');const chCpo=(c.spent&&c.sales)?Math.round(c.spent/c.sales):c.cpo
        ...
        <div style={{display:'flex',alignItems:'center',gap:4}}>
          {ce&&<button onClick={()=>remCh(ci)} style={{background:'none',border:'none',color:S.ln,cursor:'pointer',fontSize:15,padding:0,lineHeight:1}}><i className="lucide lucide-x" style={{fontSize:15}}/></button>}
      ...
      {ce&&<div style={{padding:'12px 16px'}}><button onClick={addCh} style={{fontSize:17,color:S.cta,background:'none',border:`1px dashed ${S.ln}`,borderRadius:8,padding:'10px 18px',cursor:'pointer',width:'100%',fontWeight:600}}>+ Добавить канал</button></div>}
    </div>
    ...
    <div style={{display:'flex',gap:12,marginBottom:16,flexWrap:'wrap'}}>
      <div style={{flex:1,background:S.sf,borderRadius:16,boxShadow:S.sh,padding:'22px 16px'}}><div style={{fontSize:17,fontWeight:600,color:'#3B6D11',marginBottom:6}}> Дополнения</div><EList items={rep.manual_improved} canEdit={ce} onSave={v=>up({manual_improved:v})} color="#27500A"/></div>
...
function Tactical({tasks,progress,reports,aIdx,ce,reload,profile}){
  ...
  const TaskCard=({t})=>{const tp=getTP(t.id);const ms=t.milestones||[];const history=getAllTP(t.id);let pctVal=0;if(t.target_type==='numeric'){pctVal=t.target_value?Math.round(((tp?.current_value||0)/t.target_value)*100):0}else{const cur=tp?.milestone_status||0;pctVal=ms.length?Math.round(cur/ms.length*100):0}
    return<div style={{background:S.sf,borderRadius:16,boxShadow:S.sh,padding:16,marginBottom:12}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><span style={{fontSize:15,fontWeight:500}}>{t.name}</span><div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:20,fontWeight:700,color:pctVal>=80?'#234003':pctVal>=50?'#B76400':'#A71F00'}}>{pctVal}%</span>{ce&&<button onClick={()=>toggleHide(t)} title={t.hidden?'Показать':'Скрыть'} style={{fontSize:17,background:'none',border:'none',cursor:'pointer',color:S.i3}}>{t.hidden?'':''}</button>}</div></div>
...
function Projects({projects,setProjects,comments,setComments,ce,reports,aIdx,profile,reload}){
  ...
  const ProjCard=({p})=>{const ps=PROJ_ST[p.status]||PROJ_ST.wait;const wcs=getWC(p.id);const prev=getPrev(p.id);const isO=exp===p.id;const isBlocked=p.status==='blocked';const hasBlock=p.constraints_text&&p.constraints_text.trim();const ds=dates(p)
    return<div style={{background:isBlocked?'#FFF0F0':S.sf,border:`1px solid ${isO?S.gl:isBlocked?'#E8AAAA':S.ln}`,borderRadius:14,padding:'12px 16px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10,cursor:'pointer'}} onClick={()=>setExp(isO?null:p.id)}>
        ...
        <div style={{display:'flex',gap:6,alignItems:'center'}}>{ce?<select value={p.status} onClick={e=>e.stopPropagation()} onChange={e=>upProj(p.id,'status',e.target.value)} style={{fontSize:15,padding:'2px 6px',borderRadius:20,border:`1px solid ${S.ln}`,background:ps.bg,color:ps.tx,cursor:'pointer'}}>{Object.entries(PROJ_ST).map(([k,v])=><option key={k} value={k}>{v.l}</option>)}</select>:<Chip bg={ps.bg} tx={ps.tx}>{ps.l}</Chip>}{ce&&<button onClick={e=>{e.stopPropagation();delProj(p.id,p.name)}} style={{fontSize:15,background:'none',border:'none',cursor:'pointer',color:'#ccc'}} title="Удалить"><i className="lucide lucide-trash-2" style={{fontSize:17}}/></button>}<span style={{color:S.i3}}>{isO?'▲':'▼'}</span></div>
      </div>
  ...
  return<>
    <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
      ...
      <span style={{marginLeft:'auto',display:'flex',gap:6,alignItems:'center'}}><span style={{fontSize:15,color:S.i3}}>{filtered.length} из {active.length}</span>{ce&&<button onClick={sendSurvey} disabled={surveying} style={{fontSize:15,color:'#fff',background:S.cta,border:'none',borderRadius:8,padding:'4px 12px',cursor:'pointer',fontWeight:600}} title="Запустить бота для сбора апдейтов">{surveying?'...':'Опросить команду'} <i className="lucide lucide-info" style={{fontSize:15,opacity:.7}}/></button>}{ce&&<button onClick={()=>setShowNewP(!showNewP)} style={{fontSize:15,color:S.gd,background:S.gp,border:'none',borderRadius:8,padding:'4px 12px',cursor:'pointer',fontWeight:600}}>{showNewP?'Закрыть':'+ проект'}</button>}</span>
    </div>
    {showNewP&&<div style={{background:S.sf,border:`1px solid ${S.gl}`,borderRadius:14,padding:16,marginBottom:12}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'100px 1fr 120px 120px',gap:8,marginBottom:12}}>
        <input value={np.id} onChange={e=>setNp({...np,id:e.target.value})} placeholder="ID (ST01)" style={{padding:'6px 10px',borderRadius:20,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <input value={np.name} onChange={e=>setNp({...np,name:e.target.value})} placeholder="Название проекта" style={{padding:'6px 10px',borderRadius:20,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <input value={np.owner} onChange={e=>setNp({...np,owner:e.target.value})} placeholder="Владелец" style={{padding:'6px 10px',borderRadius:20,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <select value={np.priority} onChange={e=>setNp({...np,priority:e.target.value})} style={{padding:'6px 10px',borderRadius:20,border:`1px solid ${S.ln}`,fontSize:15}}><option value="key"> Ключевой</option><option value="current"> Текущий</option></select>
      </div>
      <button onClick={addProj} disabled={!np.id.trim()||!np.name.trim()} style={{padding:'10px 18px',borderRadius:8,border:'none',background:S.cta,color:'#fff',fontSize:15,fontWeight:600,cursor:'pointer'}}>Создать</button>
    </div>}
...
function Trends({reports,mPlans}){const[mode,setMode]=useState('weekly');const[selCh,setSelCh]=useState({});const[cpoMode,setCpoMode]=useState('both');const allCh=new Set();reports.forEach(r=>(r.channels||[]).forEach(c=>{const n=c.name?.split(' (')[0]||c.name;if(n)allCh.add(n)}));const chNames=[...allCh];useEffect(()=>{if(Object.keys(selCh).length===0){const init={};chNames.forEach(n=>init[n]=true);setSelCh(init)}},[chNames.length]);const togCh=(n)=>setSelCh(p=>({...p,[n]:!p[n]}));const toggleAll=()=>{const anyOff=chNames.some(n=>!selCh[n]);const nv={};chNames.forEach(n=>nv[n]=anyOff);setSelCh(nv)}
  ...
  return<><div style={{display:'flex',gap:8,marginBottom:16,alignItems:'center',flexWrap:'wrap'}}><div style={{display:'flex',gap:2,background:S.sf,borderRadius:8,padding:2,border:`1px solid ${S.ln}`}}><button onClick={()=>setMode('weekly')} style={{padding:'6px 14px',borderRadius:10,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:mode==='weekly'?S.gd:'transparent',color:mode==='weekly'?S.gp:S.i2}}>По неделям</button><button onClick={()=>setMode('daily')} style={{padding:'6px 14px',borderRadius:10,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:mode==='daily'?S.gd:'transparent',color:mode==='daily'?S.gp:S.i2}}>По дням</button></div>{mode==='weekly'&&<div style={{display:'flex',gap:2,background:S.sf,borderRadius:8,padding:2,border:`1px solid ${S.ln}`}}>{[{k:'both',l:'CPO оба'},{k:'ads',l:'CPO Ads'},{k:'total',l:'CPO Total'}].map(o=><button key={o.k} onClick={()=>setCpoMode(o.k)} style={{padding:'6px 10px',borderRadius:10,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:cpoMode===o.k?S.am:'transparent',color:cpoMode===o.k?'#fff':S.i2}}>{o.l}</button>)}</div>}</div>
...
function Admin({profiles,projects,reports,aIdx,reload,rep,upRep}){const[pins,setPins]=useState(rep?.pinned_projects||[]);useEffect(()=>setPins(rep?.pinned_projects||[]),[rep]);const upRole=async(id,r)=>{await supabase.from('profiles').update({role:r}).eq('id',id);reload()};const upPP=async(id,p)=>{await supabase.from('projects').update({priority:p}).eq('id',id);reload()};const togPin=async pid=>{const n=pins.includes(pid)?pins.filter(x=>x!==pid):[...pins,pid];setPins(n);if(rep)await supabase.from('weekly_reports').update({pinned_projects:n}).eq('id',rep.id);reload()}
  ...
  return<><Box title={`Каналы в Обзоре · ${rep?.week_label||''}`}><div style={{fontSize:15,color:S.i3,marginBottom:12}}>Включи/выключи каналы для отображения на этой неделе.</div>{allChNames.map(name=>{const ch=(rep?.channels||[]).find(c=>c.name===name);const on=visCh.includes(name);return<div key={name} style={{display:'flex',alignItems:'center',gap:10,padding:'6px 0',borderBottom:`1px solid ${S.ln}`,fontSize:17}}><input type="checkbox" checked={on} onChange={()=>togCh(name)} style={{cursor:'pointer'}}/><span style={{flex:1,fontWeight:500,opacity:on?1:0.4}}>{name}</span><span style={{fontSize:15,color:S.i3}}>{ch?.sales||0} продаж</span>{ch?.cpo&&<span style={{fontSize:15,color:S.i2}}>CPO ${ch.cpo}</span>}</div>})}</Box>
    <Box title="Пользователи">{profiles.map(p=><div key={p.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:`1px solid ${S.ln}`}}><div><span style={{fontWeight:500}}>{p.name||p.email}</span><span style={{fontSize:15,color:S.i3,marginLeft:8}}>{p.email}</span></div><select value={p.role} onChange={e=>upRole(p.id,e.target.value)} style={{padding:'4px 10px',borderRadius:20,border:`1px solid ${S.ln}`,fontSize:17}}><option value="admin">admin</option><option value="editor">editor</option><option value="viewer">viewer</option></select></div>)}</Box>
    ...
    <div style={{background:S.sf,borderRadius:16,boxShadow:S.sh,padding:16}}><div style={{fontSize:17,fontWeight:600,marginBottom:12}}>Подключения и интеграции</div>{INTEG.map((ig,i)=>{const sc=stC[ig.st]||stC.planned;return<div key={i} style={{padding:'10px 0',borderBottom:`1px solid ${S.ln}`}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}}><div style={{flex:1}}>{ig.url?<a href={ig.url} target="_blank" rel="noopener" style={{color:S.gd,fontWeight:500,fontSize:17}}>{ig.name}</a>:<span style={{fontWeight:500,fontSize:17}}>{ig.name}</span>}<div style={{fontSize:15,color:S.i2,marginTop:2,lineHeight:1.5}}>{ig.desc}</div></div><Chip bg={sc.bg} tx={sc.tx}>{sc.l}</Chip></div></div>})}</div></>}
