...
function Overview({rep,reports,projects,comments,ce,up,tTasks,tProgress,print,refreshDaily,mPlans}){
  ...
  // CPO calcs — use spent from channels + discounts
  ...
  const svcWeek=Math.round((mPlan?.services_budget||0)/dim*7)
  const awWeek=Math.round((mPlan?.awareness_budget||0)/dim*7)
  const discBL=m.discBL||0;const discBS=m.discBS||0
  const calcCpoAds=paidSales>0?Math.round((adSpend+discBS+svcWeek+awWeek)/paidSales):null
  const calcCpoTotal=(m.totalSales||0)>0?Math.round((adSpend+discBL+svcWeek+awWeek+teamWeek)/(m.totalSales||1)):null
  ...
  return<div id="overview-print"><>
    ...
    {monthData&&(()=>{const md=monthData;const dd=md.daysWithData||1;const dim_p=mPlan?.days_in_month||dim;const svcPeriod=Math.round((mPlan?.services_budget||0)/dim_p*dd);const awPeriod=Math.round((mPlan?.awareness_budget||0)/dim_p*dd);const teamPeriod=Math.round((mPlan?.team_budget||TEAM_MONTHLY)/dim_p*dd);const calcSalesM=md.totalSales;const calcCpoAdsM=md.paidSales>0?Math.round((md.adSpend+md.discountsBS+svcPeriod+awPeriod)/md.paidSales):null;const calcCpoTotalM=md.totalSales>0?Math.round((md.adSpend+md.discountsBL+svcPeriod+awPeriod+teamPeriod)/md.totalSales):null;const calcBudgetM=md.adSpend+md.discountsBL+svcPeriod+teamPeriod;const cps=mPlan?.channel_plans||{};const ppSales=(cps.meta?.planSales||0)+(cps.google?.planSales||0)+(cps.newChannels?.planSales||0);const calcPCpoAds=ppSales>0?Math.round(((mPlan?.ads_budget||0)+(mPlan?.services_budget||0)+(mPlan?.awareness_budget||0))/ppSales):null;const calcPCpoTotal=(mPlan?.total_plan||0)>0?Math.round(((mPlan?.ads_budget||0)+(mPlan?.team_budget||0)+(mPlan?.services_budget||0)+(mPlan?.awareness_budget||0))/mPlan.total_plan):null;const calcPBudget=(mPlan?.ads_budget||0)+(mPlan?.team_budget||0)+(mPlan?.services_budget||0)+(mPlan?.awareness_budget||0)
