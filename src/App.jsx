  ...
  return<div id="overview-print"><>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:16}}>
      {[
        ...
        {l:'CPO Ads',k2:'cpoAdsOverride',v2:cpoAds,calc:calcCpoAds,pre:'$',s:planCpoAds?<>план ${planCpoAds} · <CpoCompare fact={cpoAds} plan={planCpoAds}/></>:'рекл / платные'},
        {l:'CPO Total',k2:'cpoTotalOverride',v2:cpoTotal,calc:calcCpoTotal,pre:'$',s:planCpoTotal?<>план ${planCpoTotal} · <CpoCompare fact={cpoTotal} plan={planCpoTotal}/></>:'(рекл+команда) / все'},
        ...
        <div style={{fontSize:12,color:S.i2}}>{x.s}</div>
      </div>)}
    </div>
    ...
    <div style={{background:S.sf,border:`0.5px solid ${S.ln}`,borderRadius:12,overflow:'hidden',marginBottom:16}}>
      ...
      {ch.map((c,i)=>{const barPct=c.planSales?(Math.min((c.sales||0)/c.planSales*100,100)):((c.sales||0)>0?100:0);const barColor=c.planSales?((c.sales||0)>=c.planSales?'#1D9E75':(c.sales||0)>=c.planSales*0.7?'#EF9F27':'#E24B4A'):S.gl
    ...
    <div style={{display:'flex',gap:12,marginBottom:12}}>
      <div style={{flex:1,background:'#EAF3DE',borderRadius:12,padding:'14px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
          ...
          {ce&&<button onClick={()=>{const d=JSON.stringify({week:rep.week_label,metrics:m,channels:ch,daily},null,2);navigator.clipboard?.writeText('Проанализируй данные дашборда за неделю '+rep.week_label+' и сгенерируй списки "Улучшилось" и "Ухудшилось" (3-5 пунктов каждый, кратко):\n'+d);alert('Промпт скопирован в буфер!')}} style={{fontSize:10,color:'#7C9F55',background:'none',border:`1px solid #7C9F55`,borderRadius:4,padding:'2px 8px',cursor:'pointer'}}>📋 промпт</button>}
        </div>
...
function Tactical({tasks,progress,reports,aIdx,ce,reload}){
  const rep=reports[aIdx];const ws=rep?.week_start;const[showAdd,setShowAdd]=useState(false);const[newName,setNewName]=useState('');const[newType,setNewType]=useState('numeric');const[newTarget,setNewTarget]=useState('');const[newMs,setNewMs]=useState([{name:''}]);const[showHidden,setShowHidden]=useState(false)
  ...
  const TaskCard=({t})=>{const tp=getTP(t.id);const ms=t.milestones||[];const history=getAllTP(t.id);let pctVal=0;if(t.target_type==='numeric'){pctVal=t.target_value?Math.round(((tp?.current_value||0)/t.target_value)*100):0}else{const cur=tp?.milestone_status||0;pctVal=ms.length?Math.round(cur/ms.length*100):0}
    return<div style={{background:S.sf,border:`0.5px solid ${S.ln}`,borderRadius:12,padding:16,marginBottom:12}}>
      ...
      <div style={{marginTop:8}}><Ed value={tp?.comment||''} canEdit={ce} multi onSave={v=>upsertTP(t.id,{comment:v})} ph="Комментарий за эту неделю..." style={{fontSize:13,color:S.i2,lineHeight:1.6}}/></div>
...
// ═══ TRENDS ═══
function Trends({reports}){const[mode,setMode]=useState('weekly');const[selCh,setSelCh]=useState({});const[cpoMode,setCpoMode]=useState('both');const allCh=new Set();reports.forEach(r=>(r.channels||[]).forEach(c=>{const n=c.name?.split(' (')[0]||c.name;if(n)allCh.add(n)}));const chNames=[...allCh];useEffect(()=>{if(Object.keys(selCh).length===0){const init={};chNames.forEach(n=>init[n]=true);setSelCh(init)}},[chNames.length]);const togCh=(n)=>setSelCh(p=>({...p,[n]:!p[n]}));const toggleAll=()=>{const anyOff=chNames.some(n=>!selCh[n]);const nv={};chNames.forEach(n=>nv[n]=anyOff);setSelCh(nv)}
  const weeklyData=reports.map(r=>{const m=r.metrics||{};const ch=r.channels||[];const paidSales=ch.filter(c=>(c.cpo||0)>0).reduce((s,c)=>s+(c.sales||0),0);const adSpend=ch.reduce((s,c)=>s+((c.sales||0)*(c.cpo||0)),0);const teamWeek=Math.round(TEAM_MONTHLY/30*7);const o={week:r.week_label,sales:m.totalSales,planSales:m.planSales,cpoAds:m.cpoAdsOverride!=null?m.cpoAdsOverride:(paidSales>0?Math.round(adSpend/paidSales):null),cpoTotal:m.cpoTotalOverride!=null?m.cpoTotalOverride:((m.totalSales||0)>0?Math.round((adSpend+teamWeek)/(m.totalSales||1)):null)};const planPaidSales=ch.filter(c=>(c.planCpo||0)>0).reduce((s,c)=>s+(c.planSales||0),0);const planAdSpend=ch.reduce((s,c)=>s+((c.planSales||0)*(c.planCpo||0)),0);o.planCpoAds=planPaidSales>0?Math.round(planAdSpend/planPaidSales):null;o.planCpoTotal=(m.planSales||0)>0?Math.round((planAdSpend+teamWeek)/(m.planSales||1)):null;ch.forEach(c=>{o[c.name?.split(' (')[0]||c.name]=c.sales});return o})
...
function Admin({profiles,projects,reports,aIdx,reload}){const rep=reports[aIdx];const[pins,setPins]=useState(rep?.pinned_projects||[]);useEffect(()=>setPins(rep?.pinned_projects||[]),[rep]);const upRole=async(id,r)=>{await supabase.from('profiles').update({role:r}).eq('id',id);reload()};const upPP=async(id,p)=>{await supabase.from('projects').update({priority:p}).eq('id',id);reload()};const togPin=async pid=>{const n=pins.includes(pid)?pins.filter(x=>x!==pid):[...pins,pid];setPins(n);if(rep)await supabase.from('weekly_reports').update({pinned_projects:n}).eq('id',rep.id);reload()}
  const INTEG=[{name:'Google Sheets — Daily Tracking',url:'https://docs.google.com/spreadsheets/d/1qkd5LZ-5ZHVElfrSg1GXp-s_Tiw_CgitK_N1bBxRlcw',st:'manual',desc:'Ежедневные продажи/бюджеты/CPO. Кнопка «Загрузить данные» в Обзоре.'},{name:'Google Sheets — KPI Weekly',url:'https://docs.google.com/spreadsheets/d/1qT8V2qlUWW3wROYrIILbjPGaGuX6b2TnWb6Omus8QVk',st:'manual',desc:'KPI по подразделениям.'},{name:'Стратегический дашборд',url:'https://docs.google.com/spreadsheets/d/1FQ8r5QYwENNWUSLScfRdDYKc16J8lnKG5XzPt_kgaFU',st:'connected',desc:'Мастер-список проектов. Синхронизация при деплое.'},{name:'Supabase',url:'https://lkkhwumnuzepxyvvwubd.supabase.co',st:'connected',desc:'Хранилище данных.'},{name:'Claude API',st:'planned',desc:'Автоанализ: отправка данных недели → генерация Улучшилось/Ухудшилось. Нужен ANTHROPIC_API_KEY в Vercel Env.'},{name:'Slack бот',st:'planned',desc:'Опрос команды пн 18:30. Нужен Slack App + SLACK_BOT_TOKEN.'},{name:'Meta Ads MCP',st:'planned',desc:'Импорт Meta данных. Подключён в Claude, для дашборда нужен API bridge.'},{name:'Klaviyo MCP',st:'planned',desc:'Импорт email CR. Подключён в Claude, для дашборда нужен API bridge.'}]
