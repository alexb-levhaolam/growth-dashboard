...
const S={bg:'#F7F8F9',sf:'#FFFFFF',ink:'#121416',i2:'#3E4349',i3:'#737A82',ln:'#E4E6E9',gd:'#6E9B0E',gm:'#497B02',gl:'#97C459',gs:'#C0DD97',gp:'#F7FBEA',ol:'#585E65',am:'#F18B0E',rm:'#DD2A02',bm:'#1761CB'}
const STATUS_CFG={green:{icon:'ti-circle-check',label:'зелёный',bg:'#F7FBEA',tx:'#497B02',desc:'Всё по плану'},yellow:{icon:'ti-alert-triangle',label:'жёлтый',bg:'#FDF7E3',tx:'#F18B0E',desc:'Есть отклонения'},red:{icon:'ti-alert-circle',label:'красный',bg:'#FFEEEA',tx:'#DD2A02',desc:'Критичные проблемы'}}
const PROJ_ST={done:{l:'готово',bg:'#F7FBEA',tx:'#497B02'},progress:{l:'в работе',bg:'#FDF7E3',tx:'#F18B0E'},test:{l:'тест',bg:'#EDF5FF',tx:'#1761CB'},risk:{l:'риск',bg:'#FFEEEA',tx:'#DD2A02'},wait:{l:'ожидание',bg:'#F7F8F9',tx:'#3E4349'},blocked:{l:'блокер',bg:'#FFEEEA',tx:'#DD2A02'}}
...
function Chip({children,bg,tx}){return<span style={{fontSize:15,fontWeight:600,padding:'6px 14px',borderRadius:999,background:bg,color:tx,whiteSpace:'nowrap'}}>{children}</span>}
function pct(c,p){if(!p||p===0)return null;return Math.round(((c-p)/p)*100)}
function DChip({d}){if(d==null)return<span style={{color:S.i3,fontSize:15}}>—</span>;return<span style={{color:d===0?S.i3:d>0?'#1D7A3F':'#A32D2D',fontSize:15,fontWeight:600}}>{d>0?'+':''}{d}%</span>}
function PlanChip({fact,plan}){if(!plan)return null;const p=Math.round((fact||0)/plan*100);return<span style={{fontSize:15,color:p>=90?'#1D7A3F':p>=70?'#BA7517':'#A32D2D'}}>{p}%</span>}
function CpoCompare({fact,plan}){if(!plan||!fact)return null;const d=fact-plan;const c=d<=0?'#1D7A3F':d<=30?'#BA7517':'#A32D2D';return<span style={{fontSize:15,color:c}}>{d>0?'+':''}{d}</span>}
function Label({children}){return<p style={{fontSize:15,color:S.i3,margin:'0 0 12px 2px',fontWeight:600,textTransform:'uppercase',letterSpacing:'.06em'}}>{children}</p>}
function Box({title,children}){return<div style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:'26px 18px',marginBottom:28}}><div style={{fontSize:22,fontWeight:600,marginBottom:14}}>{title}</div>{children}</div>}
function CC({title,children,right}){return<div style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:'26px 18px',marginBottom:28}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}><div style={{fontSize:22,fontWeight:600}}>{title}</div>{right&&<div>{right}</div>}</div>{children}</div>}
...
function Ed({value,onSave,canEdit,multi,ph,style={}}){const[editing,setEditing]=useState(false);const[val,setVal]=useState(value);useEffect(()=>setVal(value),[value]);if(!canEdit)return<Linkify style={style}>{value||''}</Linkify>;if(!editing)return<span style={{...style,cursor:'pointer',borderBottom:'1px dashed #E3E1D8'}} onClick={()=>setEditing(true)}>{value?<Linkify style={style}>{value}</Linkify>:<span style={style}>{ph||'(кликни)'}</span>}</span>;const save=()=>{setEditing(false);if(val!==value)onSave(val)};if(multi)return<textarea value={val||''} onChange={e=>setVal(e.target.value)} onBlur={save} autoFocus rows={2} style={{width:'100%',padding:'6px 10px',borderRadius:6,border:`1px solid ${S.gl}`,fontSize:17,resize:'vertical',outline:'none',fontFamily:'inherit',...style}}/>;return<input value={val||''} onChange={e=>setVal(e.target.value)} onBlur={save} onKeyDown={e=>e.key==='Enter'&&save()} autoFocus style={{width:'100%',padding:'4px 8px',borderRadius:6,border:`1px solid ${S.gl}`,fontSize:'inherit',outline:'none',...style}}/>}
...
function EList({items,onSave,canEdit,ph,color}){const[ni,setNi]=useState('');const add=()=>{if(!ni.trim())return;onSave([...(items||[]),ni.trim()]);setNi('')};return<div><ul style={{margin:0,paddingLeft:18,fontSize:17,lineHeight:1.75,color:color||'inherit'}}>{(items||[]).map((t,i)=><li key={i} style={{display:'flex',alignItems:'flex-start',gap:6}}><Ed value={t} canEdit={canEdit} onSave={v=>{const n=[...(items||[])];n[i]=v;onSave(n)}} style={{flex:1,fontSize:17,color:'inherit'}}/>{canEdit&&<button onClick={()=>{const n=[...(items||[])];n.splice(i,1);onSave(n)}} style={{background:'none',border:'none',color:'#ccc',cursor:'pointer',fontSize:17,padding:0}}>×</button>}</li>)}</ul>{canEdit&&<div style={{display:'flex',gap:6,marginTop:6}}><input value={ni} onChange={e=>setNi(e.target.value)} placeholder={ph||'Добавить...'} onKeyDown={e=>e.key==='Enter'&&add()} style={{flex:1,padding:'4px 10px',borderRadius:6,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/><button onClick={add} style={{padding:'4px 12px',borderRadius:6,border:'none',background:S.gd,color:S.gp,fontSize:15,cursor:'pointer'}}>+</button></div>}</div>}

function Login(){const[e,setE]=useState('');const[p,setP]=useState('');const[err,setErr]=useState('');const[ld,setLd]=useState(false);const go=async()=>{setLd(true);setErr('');const{error}=await supabase.auth.signInWithPassword({email:e,password:p});if(error)setErr(error.message);setLd(false)};return<div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:S.bg}}><div style={{background:S.sf,borderRadius:16,padding:32,width:360,border:`1px solid ${S.ln}`}}><div style={{textAlign:'center',marginBottom:24}}><div style={{fontSize:24,fontWeight:600,color:S.gd}}>Growth Dashboard</div><div style={{fontSize:17,color:S.i3,marginTop:4}}>Lev Haolam</div></div><input value={e} onChange={ev=>setE(ev.target.value)} placeholder="Email" style={{width:'100%',padding:'10px 14px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:17,marginBottom:10,outline:'none'}}/><input value={p} onChange={ev=>setP(ev.target.value)} placeholder="Пароль" type="password" onKeyDown={ev=>ev.key==='Enter'&&go()} style={{width:'100%',padding:'10px 14px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:17,marginBottom:14,outline:'none'}}/>{err&&<div style={{color:S.rm,fontSize:17,marginBottom:10}}>{err}</div>}<button onClick={go} disabled={ld} style={{width:'100%',padding:'11px',borderRadius:8,border:'none',background:S.gd,color:S.gp,fontSize:17,fontWeight:600,cursor:'pointer'}}>{ld?'...':'Войти'}</button></div></div>}
    ...
    <div style={{background:S.gd,borderRadius:14,padding:'18px 22px',marginBottom:16}} className="no-print">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <button onClick={()=>supabase.auth.signOut()} style={{fontSize:15,color:S.gs,background:'rgba(255,255,255,0.15)',border:'none',borderRadius:6,padding:'4px 10px',cursor:'pointer'}}>Выйти</button>
        </div>
      </div>
      {rep&&<><h1 style={{margin:'4px 0 0',fontSize:24,color:S.gp,fontFamily:'Georgia,serif',fontWeight:400}}>Неделя <Ed value={rep.week_label} canEdit={ce} onSave={v=>upRep({week_label:v})} style={{fontSize:24,color:S.gp,fontFamily:'Georgia,serif',fontWeight:400}}/></h1>
        ...
        <div style={{marginTop:10,display:'flex',gap:4,flexWrap:'wrap',alignItems:'center'}}>
          {reports.map((r,i)=><button key={r.id} onClick={()=>setWeek(i)} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:i===aIdx?S.gp:'rgba(255,255,255,0.15)',color:i===aIdx?S.gd:S.gs}}>{r.week_label}</button>)}
          ...
          {isA&&rep&&<button onClick={deleteWeek} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:15,background:'rgba(224,75,74,0.3)',color:'#FFC1C1'}} title="Удалить текущий отчёт">🗑</button>}
        </div>
    </div>
  ...
  return<div id="overview-print"><>
    <div style={{display:'flex',gap:8,marginBottom:14,fontSize:15,color:S.i3,flexWrap:'wrap',alignItems:'center'}}>
      ...
      <span style={{marginLeft:'auto',display:'flex',gap:4}}>
        {ce&&<button onClick={()=>setShowPaste(true)} style={{fontSize:15,color:S.gd,background:S.gp,border:`1px solid ${S.ln}`,borderRadius:6,padding:'3px 10px',cursor:'pointer'}}>📊 Вставить вручную</button>}
        {ce&&refreshDaily&&<button onClick={refreshDaily} style={{fontSize:15,color:S.sf,background:S.bm,border:'none',borderRadius:6,padding:'3px 10px',cursor:'pointer'}}>📥 Из Daily Sheet</button>}
        {ce&&<button onClick={refreshTotals} style={{fontSize:15,color:S.bm,background:S.sf,border:`1px solid ${S.ln}`,borderRadius:6,padding:'3px 10px',cursor:'pointer'}}>🔄 Обновить итоги</button>}
        <button onClick={print} style={{fontSize:15,color:S.gd,background:S.sf,border:`1px solid ${S.ln}`,borderRadius:6,padding:'3px 10px',cursor:'pointer'}}>📄 PDF</button>
      </span>
    </div>

    {showPaste&&<div style={{background:S.sf,border:`1px solid ${S.gl}`,borderRadius:14,padding:16,marginBottom:16}}>
      <div style={{fontSize:17,fontWeight:600,marginBottom:6}}>Вставь данные из Google Sheets</div>
      <div style={{fontSize:15,color:S.i3,marginBottom:8}}>Формат: каждая строка = день. Колонки через TAB: День | Продажи | Заметка</div>
      ...
      <div style={{display:'flex',gap:8,marginTop:8}}>
        <button onClick={applyPaste} style={{padding:'8px 20px',borderRadius:8,border:'none',background:S.gd,color:S.gp,fontSize:17,fontWeight:600,cursor:'pointer'}}>Применить</button>
    ...
    return<div style={{background:'linear-gradient(135deg,#0F6E56 0%,#1D9E75 100%)',borderRadius:14,padding:'14px 20px',marginBottom:16,color:S.gp}}>
      <div style={{fontSize:15,fontWeight:600,letterSpacing:'.05em',textTransform:'uppercase',color:S.gs,marginBottom:8}}>📊 Итоги месяца · {md.daysWithData} дней</div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:10,marginBottom:16}}>
      ...
      ].map((x,i)=><div key={i} style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:10,padding:'12px 14px'}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(7,minmax(80px,1fr))',gap:6,marginBottom:16,overflowX:'auto'}}>
      {(daily.length>0?daily:Array(7).fill(null)).map((d,i)=>{const chTip=d?.ch?Object.entries(d.ch).filter(([,v])=>v>0).map(([k,v])=>`${k}: ${v}`).join('\n'):'';return<div key={i} title={chTip} style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:10,padding:'8px 4px',textAlign:'center',cursor:chTip?'help':'default'}}>
        ...
        <Ed value={d?.note||(d?.sales==null?'—':'продаж')} canEdit={ce} onSave={v=>upDay(i,'note',v)} style={{fontSize:15,color:d?.sales==null?'#BA7517':S.i2}}/>
      </div>})}
    </div>

    <Label>Каналы</Label>
    <div style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,overflow:'hidden',marginBottom:16}}>
      <div style={{display:'grid',gridTemplateColumns:'110px 1fr 46px 46px 38px 38px 52px 52px 38px',gap:3,padding:'8px 12px',background:S.bg,fontSize:15,color:S.i3,fontWeight:600,textTransform:'uppercase'}}>
      ...
      {ch.filter(c=>!visCh||visCh.includes(c.name)).map((c,i)=>{const ci=ch.indexOf(c);const mp=getChPlan(c.name);const ps=c.planSales!=null?c.planSales:(mp.planSales||0);const barPct=ps?(Math.min((c.sales||0)/ps*100,100)):((c.sales||0)>0?100:0);const barColor=ps?((c.sales||0)>=ps?'#1D9E75':(c.sales||0)>=ps*0.7?'#EF9F27':'#E24B4A'):((c.sales||0)>0?S.gl:'#E3E1D8');const chCpo=(c.spent&&c.sales)?Math.round(c.spent/c.sales):c.cpo
        return<div key={i} style={{display:'grid',gridTemplateColumns:'110px 1fr 46px 46px 38px 38px 52px 52px 38px',gap:3,padding:'5px 12px',borderBottom:`1px solid ${S.ln}`,alignItems:'center',fontSize:17}}>
        <div style={{display:'flex',alignItems:'center',gap:3}}>
          {ce&&<button onClick={()=>remCh(ci)} style={{background:'none',border:'none',color:'#ccc',cursor:'pointer',fontSize:15,padding:0,lineHeight:1}}>×</button>}
          <Ed value={c.name} canEdit={ce} onSave={v=>upCh(ci,'name',v)} style={{fontWeight:500,fontSize:15}}/>
        </div>
        ...
        <span style={{textAlign:'right'}}><EdNum value={c.sales} canEdit={ce} onSave={v=>upCh(ci,'sales',v)} style={{fontWeight:500,fontSize:17}}/></span>
        ...
        <span style={{textAlign:'right'}}><EdNum value={chCpo} canEdit={ce} prefix="$" onSave={v=>upCh(ci,'cpo',v)} style={{fontSize:15,color:S.i2}}/></span>
      ...
      {ce&&<div style={{padding:'8px 12px'}}><button onClick={addCh} style={{fontSize:15,color:S.gd,background:'none',border:`1px dashed ${S.ln}`,borderRadius:8,padding:'6px 14px',cursor:'pointer',width:'100%'}}>+ Добавить канал</button></div>}
    </div>
    ...
    <div style={{display:'flex',gap:12,marginBottom:12,flexWrap:'wrap'}}>
      <div style={{flex:1,background:'#EAF3DE',borderRadius:14,padding:'14px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
          <div style={{fontSize:15,color:'#7C9F55',fontWeight:600}}>🤖 Анализ · Улучшилось</div>
          {ce&&<button onClick={runAnalysis} disabled={analyzing} style={{fontSize:15,color:'#fff',background:'#7C9F55',border:'none',borderRadius:4,padding:'3px 10px',cursor:'pointer',fontWeight:600}}>{analyzing?'⏳ анализ...':'🤖 Запустить анализ'}</button>}
        </div>
        <EList items={rep.improved} canEdit={ce} onSave={v=>up({improved:v})} color="#27500A"/>
      </div>
      <div style={{flex:1,background:'#FCEBEB',borderRadius:14,padding:'14px 16px'}}>
        <div style={{fontSize:15,color:'#C77',fontWeight:600,marginBottom:6}}>🤖 Анализ · Ухудшилось</div>
        <EList items={rep.worsened} canEdit={ce} onSave={v=>up({worsened:v})} color="#791F1F"/>
      </div>
    </div>
    {m.aiInsights&&<div style={{background:'#F0EAFC',borderRadius:14,padding:'14px 16px',marginBottom:12}}><div style={{fontSize:15,color:'#7B5EA7',fontWeight:600,marginBottom:4}}>💡 Инсайты от Claude</div><Linkify style={{fontSize:17,color:'#4A3372',lineHeight:1.6}}>{m.aiInsights}</Linkify></div>}
    <div style={{display:'flex',gap:12,marginBottom:16,flexWrap:'wrap'}}>
      <div style={{flex:1,background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:'14px 16px'}}><div style={{fontSize:17,fontWeight:600,color:'#3B6D11',marginBottom:6}}>✏️ Дополнения</div><EList items={rep.manual_improved} canEdit={ce} onSave={v=>up({manual_improved:v})} color="#27500A"/></div>
      <div style={{flex:1,background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:'14px 16px'}}><div style={{fontSize:17,fontWeight:600,color:'#A32D2D',marginBottom:6}}>✏️ Дополнения</div><EList items={rep.manual_worsened} canEdit={ce} onSave={v=>up({manual_worsened:v})} color="#791F1F"/></div>
    </div>
    <div style={{background:'#D6E8FA',borderRadius:14,padding:'14px 16px',marginBottom:12}}><div style={{fontSize:17,fontWeight:600,color:'#0C447C',marginBottom:6}}>Фокус на следующую неделю</div><EList items={rep.focus} canEdit={ce} onSave={v=>up({focus:v})} color="#0C447C"/></div>
    <div style={{background:S.gd,borderRadius:14,padding:'14px 16px'}}><div style={{fontSize:17,fontWeight:600,color:S.gs,marginBottom:6}}>Нужно от руководства</div><div style={{color:S.gp}}><EList items={rep.asks} canEdit={ce} onSave={v=>up({asks:v})}/></div></div>
  </></div>
}
...
function Tactical({tasks,progress,reports,aIdx,ce,reload,profile}){
  ...
  const TaskCard=({t})=>{const tp=getTP(t.id);const ms=t.milestones||[];const history=getAllTP(t.id);let pctVal=0;if(t.target_type==='numeric'){pctVal=t.target_value?Math.round(((tp?.current_value||0)/t.target_value)*100):0}else{const cur=tp?.milestone_status||0;pctVal=ms.length?Math.round(cur/ms.length*100):0}
    return<div style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:16,marginBottom:12}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}><span style={{fontSize:15,fontWeight:500}}>{t.name}</span><div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:20,fontWeight:700,color:pctVal>=80?'#1D7A3F':pctVal>=50?'#BA7517':'#A32D2D'}}>{pctVal}%</span>{ce&&<button onClick={()=>toggleHide(t)} title={t.hidden?'Показать':'Скрыть'} style={{fontSize:17,background:'none',border:'none',cursor:'pointer',color:S.i3}}>{t.hidden?'👁':'🙈'}</button>}</div></div>
  ...
  return<><div style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:'12px 16px',marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:17,color:S.i2}}>📅 <b>{rep?.week_label||'—'}</b></span><div style={{display:'flex',gap:8,alignItems:'center'}}><span style={{fontSize:17,color:daysLeft<180?S.rm:S.i2,fontWeight:600}}>До дедлайна: {daysLeft} дней</span>{ce&&<button onClick={()=>setShowAdd(!showAdd)} style={{fontSize:15,color:S.gd,background:S.gp,border:'none',borderRadius:8,padding:'4px 12px',cursor:'pointer',fontWeight:600}}>{showAdd?'✕':'+ задача'}</button>}</div></div>
    {showAdd&&<div style={{background:S.sf,border:`1px solid ${S.gl}`,borderRadius:14,padding:16,marginBottom:16}}><div style={{fontSize:17,fontWeight:600,marginBottom:10}}>Новая задача</div><input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="Название" style={{width:'100%',padding:'8px 12px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:17,outline:'none',marginBottom:8}}/><div style={{display:'flex',gap:8,marginBottom:8}}><button onClick={()=>setNewType('numeric')} style={{flex:1,padding:'8px',borderRadius:8,border:`1px solid ${newType==='numeric'?S.gd:S.ln}`,background:newType==='numeric'?S.gp:'transparent',color:newType==='numeric'?S.gd:S.i2,fontSize:15,cursor:'pointer',fontWeight:600}}>🔢 Цифровая</button><button onClick={()=>setNewType('milestone')} style={{flex:1,padding:'8px',borderRadius:8,border:`1px solid ${newType==='milestone'?S.gd:S.ln}`,background:newType==='milestone'?S.gp:'transparent',color:newType==='milestone'?S.gd:S.i2,fontSize:15,cursor:'pointer',fontWeight:600}}>🏁 Этапы</button></div>{newType==='numeric'&&<input value={newTarget} onChange={e=>setNewTarget(e.target.value)} placeholder="Целевое значение" type="number" style={{width:'100%',padding:'8px 12px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:17,outline:'none',marginBottom:8}}/>}{newType==='milestone'&&<div style={{marginBottom:8}}>{newMs.map((m,i)=><div key={i} style={{display:'flex',gap:6,marginBottom:4}}><input value={m.name} onChange={e=>{const u=[...newMs];u[i]={name:e.target.value};setNewMs(u)}} placeholder={`Этап ${i+1}`} style={{flex:1,padding:'6px 10px',borderRadius:6,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>{newMs.length>1&&<button onClick={()=>setNewMs(newMs.filter((_,j)=>j!==i))} style={{background:'none',border:'none',color:'#ccc',cursor:'pointer'}}>×</button>}</div>)}<button onClick={()=>setNewMs([...newMs,{name:''}])} style={{fontSize:15,color:S.gd,background:'none',border:`1px dashed ${S.ln}`,borderRadius:6,padding:'4px 12px',cursor:'pointer',width:'100%'}}>+ этап</button></div>}<button onClick={addTask} disabled={!newName.trim()} style={{padding:'8px 20px',borderRadius:8,border:'none',background:S.gd,color:S.gp,fontSize:17,fontWeight:600,cursor:'pointer'}}>Создать</button></div>}
...
function Projects({projects,setProjects,comments,setComments,ce,reports,aIdx,profile,reload}){
    ...
    return<div style={{background:isBlocked?'#FFF0F0':S.sf,border:`1px solid ${isO?S.gl:isBlocked?'#E8AAAA':S.ln}`,borderRadius:14,padding:'12px 16px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10,cursor:'pointer'}} onClick={()=>setExp(isO?null:p.id)}>
        <div style={{flex:1}}>
          ...
          {hasBlock&&!isO&&<div style={{fontSize:15,color:'#791F1F',marginTop:2}}>⚠️ {p.constraints_text.slice(0,60)}{p.constraints_text.length>60?'…':''}</div>}
        </div>
      ...
      {wcs.length===0&&prev&&<div style={{marginTop:6,padding:'8px 12px',background:'#FFF9E6',borderRadius:8,border:'1px dashed #EAD89B'}}><div style={{fontSize:15,color:'#BA7517'}}>⏮ {prev.week_start} ({prev.author})</div><div style={{fontSize:17,color:S.i3,fontStyle:'italic',whiteSpace:'pre-wrap'}}><Linkify>{prev.full_text||prev.summary}</Linkify></div></div>}
      {isO&&<div onClick={e=>e.stopPropagation()} style={{marginTop:12,borderTop:`1px solid ${S.ln}`,paddingTop:12}}>
        ...
        {ce&&<div style={{display:'flex',gap:8}}><textarea id={"nc-"+p.id} defaultValue="" onChange={e=>{ncRef.current=e.target.value}} placeholder={`Комментарий за ${rep?.week_label||''}...`} rows={2} style={{flex:1,padding:'8px 12px',borderRadius:8,border:`1px solid ${S.ln}`,fontSize:17,resize:'vertical',outline:'none',fontFamily:'inherit'}}/><button onClick={()=>addC(p.id)} disabled={sav} style={{padding:'8px 16px',borderRadius:8,border:'none',background:S.gd,color:S.gp,fontSize:17,fontWeight:600,cursor:'pointer',alignSelf:'flex-end'}}>{sav?'...':'→'}</button></div>}
      </div>}
    </div>}
  ...
  return<>
    <div style={{display:'flex',gap:6,marginBottom:12,flexWrap:'wrap',alignItems:'center'}}>
      ...
      {[{k:'all',l:'Все'},{k:'key',l:'Ключевые'},{k:'current',l:'Текущие'}].map(f=><button key={f.k} onClick={()=>setFPri(f.k)} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:fPri===f.k?S.gd:'#F1EFE8',color:fPri===f.k?S.gp:S.i2}}>{f.l}</button>)}
      <span style={{color:S.ln}}>|</span>
      {[{k:'all',l:'Все статусы'},...Object.entries(PROJ_ST).filter(([k])=>k!=='done').map(([k,v])=>({k,l:v.l}))].map(f=><button key={f.k} onClick={()=>setFSt(f.k)} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:fSt===f.k?S.gd:'#F1EFE8',color:fSt===f.k?S.gp:S.i2}}>{f.l}</button>)}
      <span style={{marginLeft:'auto',display:'flex',gap:6,alignItems:'center'}}><span style={{fontSize:15,color:S.i3}}>{filtered.length} из {active.length}</span>{ce&&<button onClick={sendSurvey} disabled={surveying} style={{fontSize:15,color:'#fff',background:S.bm,border:'none',borderRadius:8,padding:'4px 12px',cursor:'pointer',fontWeight:600}}>{surveying?'⏳...':'💬 Собрать апдейты'}</button>}{ce&&<button onClick={()=>setShowNewP(!showNewP)} style={{fontSize:15,color:S.gd,background:S.gp,border:'none',borderRadius:8,padding:'4px 12px',cursor:'pointer',fontWeight:600}}>{showNewP?'✕':'+ проект'}</button>}</span>
    </div>
    {showNewP&&<div style={{background:S.sf,border:`1px solid ${S.gl}`,borderRadius:14,padding:16,marginBottom:12}}>
      <div style={{fontSize:17,fontWeight:600,marginBottom:8}}>Новый проект</div>
      <div style={{display:'grid',gridTemplateColumns:'100px 1fr 120px 120px',gap:8,marginBottom:8}}>
        <input value={np.id} onChange={e=>setNp({...np,id:e.target.value})} placeholder="ID (ST01)" style={{padding:'6px 10px',borderRadius:6,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <input value={np.name} onChange={e=>setNp({...np,name:e.target.value})} placeholder="Название проекта" style={{padding:'6px 10px',borderRadius:6,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <input value={np.owner} onChange={e=>setNp({...np,owner:e.target.value})} placeholder="Владелец" style={{padding:'6px 10px',borderRadius:6,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <select value={np.priority} onChange={e=>setNp({...np,priority:e.target.value})} style={{padding:'6px 10px',borderRadius:6,border:`1px solid ${S.ln}`,fontSize:15}}><option value="key">🔴 Ключевой</option><option value="current">🔵 Текущий</option></select>
      </div>
      <button onClick={addProj} disabled={!np.id.trim()||!np.name.trim()} style={{padding:'6px 16px',borderRadius:8,border:'none',background:S.gd,color:S.gp,fontSize:15,fontWeight:600,cursor:'pointer'}}>Создать</button>
    </div>}
...
function CItem({c,ce,reload,onDel}){const[es,setEs]=useState(false);const[sv,setSv]=useState(c.full_text||c.summary);const saveS=async()=>{setEs(false);await supabase.from('project_comments').update({full_text:sv,summary:sv.length>120?sv.slice(0,117)+'…':sv}).eq('id',c.id);reload()};return<div style={{marginTop:6,padding:'8px 12px',background:S.bg,borderRadius:8}}><div style={{display:'flex',justifyContent:'space-between',fontSize:15,color:S.i3,marginBottom:2}}><span><b>{c.author}</b> · {c.week_start}</span><div style={{display:'flex',gap:6}}>{ce&&<button onClick={()=>setEs(!es)} style={{fontSize:15,color:S.bm,background:'none',border:'none',cursor:'pointer'}}>✏️</button>}{ce&&onDel&&<button onClick={onDel} style={{fontSize:15,color:'#ccc',background:'none',border:'none',cursor:'pointer'}}>🗑</button>}</div></div>{es?<div><textarea value={sv} onChange={e=>setSv(e.target.value)} rows={3} style={{width:'100%',padding:'6px 10px',borderRadius:6,border:`1px solid ${S.gl}`,fontSize:17,outline:'none',fontFamily:'inherit',resize:'vertical'}}/><button onClick={saveS} style={{fontSize:15,color:S.gd,background:'none',border:'none',cursor:'pointer'}}>💾</button></div>:<div style={{fontSize:17,color:S.i2,whiteSpace:'pre-wrap',lineHeight:1.5}}><Linkify>{c.full_text||c.summary}</Linkify></div>}</div>}
...
function Dynamics({projects,comments,reports,aIdx,ce,reload}){const[expanded,setExpanded]=useState(null);const[fSt,setFSt]=useState('all');const rep=reports[aIdx]
  ...
  const TL=({proj})=>{const isOpen=expanded===proj.id;const ps=PROJ_ST[proj.status]||PROJ_ST.wait;const pC=comments.filter(c=>c.project_id===proj.id);const weeks=[...reports].reverse().map(r=>{const snap=getSnap(proj.id,r.week_start);const snapSt=snap?PROJ_ST[snap.status]:null;return{...r,wc:pC.filter(c=>c.week_start===r.week_start),snap,snapSt}})
    ...
    return<div style={{background:hasBlock?'#FFF0F0':S.sf,border:`1px solid ${hasBlock?'#E8AAAA':S.ln}`,borderRadius:14,marginBottom:8}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',padding:'12px 16px',cursor:'pointer'}} onClick={()=>setExpanded(isOpen?null:proj.id)}>
        <div style={{flex:1}}>
          ...
          {hasBlock&&<div style={{fontSize:15,color:'#791F1F',marginTop:2}}>⚠️ {proj.constraints_text}</div>}
        </div>
      ...
      {isOpen&&<div style={{padding:'0 16px 16px',borderLeft:`3px solid ${S.gl}`,marginLeft:16}}>{weeks.map(w=><div key={w.id} style={{marginTop:12,paddingBottom:12,borderBottom:`1px solid ${S.ln}`}}>
        ...
        {w.snap&&<div style={{fontSize:15,color:S.i3,marginTop:2}}>{w.snap.constraints&&<span style={{color:'#791F1F'}}>⚠️ {w.snap.constraints} · </span>}{[w.snap.date_start&&`начало: ${w.snap.date_start}`,w.snap.date_done&&`завершён: ${w.snap.date_done}`].filter(Boolean).join(' · ')}</div>}
  ...
  <div style={{display:'flex',gap:4,marginBottom:12,flexWrap:'wrap'}}>
    {[{k:'all',l:'Все'},...Object.entries(PROJ_ST).map(([k,v])=>({k,l:v.l}))].map(f=><button key={f.k} onClick={()=>setFSt(f.k)} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:fSt===f.k?S.gd:'#F1EFE8',color:fSt===f.k?S.gp:S.i2}}>{f.l}</button>)}
  </div>
...
function Trends({reports,mPlans}){const[mode,setMode]=useState('weekly');const[selCh,setSelCh]=useState({});const[cpoMode,setCpoMode]=useState('both');const allCh=new Set();reports.forEach(r=>(r.channels||[]).forEach(c=>{const n=c.name?.split(' (')[0]||c.name;if(n)allCh.add(n)}));const chNames=[...allCh];useEffect(()=>{if(Object.keys(selCh).length===0){const init={};chNames.forEach(n=>init[n]=true);setSelCh(init)}},[chNames.length]);const togCh=(n)=>setSelCh(p=>({...p,[n]:!p[n]}));const toggleAll=()=>{const anyOff=chNames.some(n=>!selCh[n]);const nv={};chNames.forEach(n=>nv[n]=anyOff);setSelCh(nv)}
  ...
  return<><div style={{display:'flex',gap:8,marginBottom:16,alignItems:'center',flexWrap:'wrap'}}><div style={{display:'flex',gap:2,background:S.sf,borderRadius:8,padding:2,border:`1px solid ${S.ln}`}}><button onClick={()=>setMode('weekly')} style={{padding:'6px 14px',borderRadius:6,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:mode==='weekly'?S.gd:'transparent',color:mode==='weekly'?S.gp:S.i2}}>По неделям</button><button onClick={()=>setMode('daily')} style={{padding:'6px 14px',borderRadius:6,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:mode==='daily'?S.gd:'transparent',color:mode==='daily'?S.gp:S.i2}}>По дням</button></div>{mode==='weekly'&&<div style={{display:'flex',gap:2,background:S.sf,borderRadius:8,padding:2,border:`1px solid ${S.ln}`}}>{[{k:'both',l:'CPO оба'},{k:'ads',l:'CPO Ads'},{k:'total',l:'CPO Total'}].map(o=><button key={o.k} onClick={()=>setCpoMode(o.k)} style={{padding:'6px 10px',borderRadius:6,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:cpoMode===o.k?S.am:'transparent',color:cpoMode===o.k?'#fff':S.i2}}>{o.l}</button>)}</div>}</div>
    <CC title="Продажи (факт vs план)"><ResponsiveContainer width="100%" height={220}><BarChart data={data}><CartesianGrid strokeDasharray="3 3" stroke={S.ln}/><XAxis dataKey={xKey} tick={{fontSize:15,fill:S.i3}} interval={mode==='daily'?2:0}/><YAxis tick={{fontSize:15,fill:S.i3}}/><Tooltip contentStyle={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:8,fontSize:15}}/><Bar dataKey="sales" fill={S.gm} radius={[4,4,0,0]} name="Факт"/>{mode==='weekly'&&<Bar dataKey="planSales" fill={S.gs} radius={[4,4,0,0]} name="План"/>}</BarChart></ResponsiveContainer></CC>
    {mode==='weekly'&&<CC title="CPO" right={<span style={{fontSize:15,color:S.i3}}>Пунктир = план</span>}><ResponsiveContainer width="100%" height={220}><LineChart data={weeklyData}><CartesianGrid strokeDasharray="3 3" stroke={S.ln}/><XAxis dataKey="week" tick={{fontSize:15,fill:S.i3}}/><YAxis tick={{fontSize:15,fill:S.i3}} domain={[0,'auto']}/><Tooltip contentStyle={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:8,fontSize:15}}/>{(cpoMode==='both'||cpoMode==='ads')&&<Line type="monotone" dataKey="cpoAds" stroke={S.am} strokeWidth={2} dot={{r:4,fill:S.am}} name="CPO Ads"/>}{(cpoMode==='both'||cpoMode==='total')&&<Line type="monotone" dataKey="cpoTotal" stroke={S.rm} strokeWidth={2} dot={{r:4,fill:S.rm}} name="CPO Total"/>}{(cpoMode==='both'||cpoMode==='ads')&&<Line type="monotone" dataKey="planCpoAds" stroke={S.am} strokeWidth={1} strokeDasharray="5 5" dot={false} name="План Ads"/>}{(cpoMode==='both'||cpoMode==='total')&&<Line type="monotone" dataKey="planCpoTotal" stroke={S.rm} strokeWidth={1} strokeDasharray="5 5" dot={false} name="План Total"/>}<Legend wrapperStyle={{fontSize:15}}/></LineChart></ResponsiveContainer></CC>}
    {mode==='weekly'&&<><div style={{display:'flex',gap:6,marginBottom:8,flexWrap:'wrap',alignItems:'center'}}><button onClick={toggleAll} style={{fontSize:15,color:S.gd,background:'none',border:`1px solid ${S.ln}`,borderRadius:6,padding:'3px 8px',cursor:'pointer'}}>все/ничего</button>{chNames.map(n=><label key={n} style={{display:'flex',alignItems:'center',gap:3,fontSize:15,color:S.i2,cursor:'pointer'}}><input type="checkbox" checked={!!selCh[n]} onChange={()=>togCh(n)} style={{cursor:'pointer'}}/><span style={{width:8,height:8,borderRadius:2,background:CH_COLORS[n]||S.i3,display:'inline-block'}}/>{n}</label>)}</div><CC title="Каналы"><ResponsiveContainer width="100%" height={250}><BarChart data={weeklyData}><CartesianGrid strokeDasharray="3 3" stroke={S.ln}/><XAxis dataKey="week" tick={{fontSize:15,fill:S.i3}}/><YAxis tick={{fontSize:15,fill:S.i3}}/><Tooltip contentStyle={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:8,fontSize:15}}/><Legend wrapperStyle={{fontSize:15}}/>{chNames.filter(n=>selCh[n]).map(n=><Bar key={n} dataKey={n} stackId="a" fill={CH_COLORS[n]||S.i3}/>)}</BarChart></ResponsiveContainer></CC></>}
  </>}
...
function Plan({plans,ce,reload}){
  ...
  return<><div style={{display:'flex',gap:4,marginBottom:16,flexWrap:'wrap'}}>
    {plans.map(p=><button key={p.id} onClick={()=>setSel(p.id)} style={{padding:'6px 14px',borderRadius:14,border:'none',cursor:'pointer',fontSize:15,fontWeight:600,background:sel===p.id?S.gd:'#F1EFE8',color:sel===p.id?S.gp:S.i2}}>{p.month_label}</button>)}
  </div>
...
function Admin({profiles,projects,reports,aIdx,reload,rep,upRep}){const[pins,setPins]=useState(rep?.pinned_projects||[]);useEffect(()=>setPins(rep?.pinned_projects||[]),[rep]);const upRole=async(id,r)=>{await supabase.from('profiles').update({role:r}).eq('id',id);reload()};const upPP=async(id,p)=>{await supabase.from('projects').update({priority:p}).eq('id',id);reload()};const togPin=async pid=>{const n=pins.includes(pid)?pins.filter(x=>x!==pid):[...pins,pid];setPins(n);if(rep)await supabase.from('weekly_reports').update({pinned_projects:n}).eq('id',rep.id);reload()}
  ...
  return<><Box title={`Каналы в Обзоре · ${rep?.week_label||''}`}><div style={{fontSize:15,color:S.i3,marginBottom:8}}>Включи/выключи каналы для отображения на этой неделе.</div>{allChNames.map(name=>{const ch=(rep?.channels||[]).find(c=>c.name===name);const on=visCh.includes(name);return<div key={name} style={{display:'flex',alignItems:'center',gap:10,padding:'6px 0',borderBottom:`1px solid ${S.ln}`,fontSize:17}}><input type="checkbox" checked={on} onChange={()=>togCh(name)} style={{cursor:'pointer'}}/><span style={{flex:1,fontWeight:500,opacity:on?1:0.4}}>{name}</span><span style={{fontSize:15,color:S.i3}}>{ch?.sales||0} продаж</span>{ch?.cpo&&<span style={{fontSize:15,color:S.i2}}>CPO ${ch.cpo}</span>}</div>})}</Box>
    <Box title="Пользователи">{profiles.map(p=><div key={p.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:`1px solid ${S.ln}`}}><div><span style={{fontWeight:500}}>{p.name||p.email}</span><span style={{fontSize:15,color:S.i3,marginLeft:8}}>{p.email}</span></div><select value={p.role} onChange={e=>upRole(p.id,e.target.value)} style={{padding:'4px 10px',borderRadius:6,border:`1px solid ${S.ln}`,fontSize:17}}><option value="admin">admin</option><option value="editor">editor</option><option value="viewer">viewer</option></select></div>)}</Box>
    <Box title={`Проекты на главной · ${rep?.week_label||''}`}><div style={{fontSize:15,color:S.i3,marginBottom:8}}>На каждой неделе свой набор.</div>{projects.map(p=><div key={p.id} style={{display:'flex',alignItems:'center',gap:10,padding:'5px 0',borderBottom:`1px solid ${S.ln}`,fontSize:17}}><input type="checkbox" checked={pins.includes(p.id)} onChange={()=>togPin(p.id)} style={{cursor:'pointer'}}/><span style={{flex:1}}>{p.name}</span><Chip bg={p.priority==='key'?'#F7C1C1':'#B5D4F4'} tx={p.priority==='key'?'#791F1F':'#0C447C'}>{p.priority==='key'?'key':'current'}</Chip></div>)}</Box>
    ...
    <div style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:16}}><div style={{fontSize:17,fontWeight:600,marginBottom:12}}>Подключения и интеграции</div>{INTEG.map((ig,i)=>{const sc=stC[ig.st]||stC.planned;return<div key={i} style={{padding:'10px 0',borderBottom:`1px solid ${S.ln}`}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}}><div style={{flex:1}}>{ig.url?<a href={ig.url} target="_blank" rel="noopener" style={{color:S.gd,fontWeight:500,fontSize:17}}>{ig.name}</a>:<span style={{fontWeight:500,fontSize:17}}>{ig.name}</span>}<div style={{fontSize:15,color:S.i2,marginTop:2,lineHeight:1.5}}>{ig.desc}</div></div><Chip bg={sc.bg} tx={sc.tx}>{sc.l}</Chip></div></div>})}</div></>}
