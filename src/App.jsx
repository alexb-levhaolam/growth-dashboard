...
function Main({profile}){
  ...
  return<div style={{minHeight:'100vh',background:S.bg}}>
    ...
    <div style={{background:'linear-gradient(180deg,#0F6E5E,#0B5548)',borderRadius:14,padding:'32px 32px 28px',marginBottom:32}} className="no-print">
...
function Overview({rep,reports,projects,comments,ce,up,tTasks,tProgress,print,refreshDaily,mPlans}){
  ...
  return<div id="overview-print"><>
    ...
    <div style={{background:S.sf,borderRadius:14,boxShadow:S.sh,overflow:'hidden',marginBottom:28}}>
      <div style={{display:'grid',gridTemplateColumns:'110px 160px 70px 60px 55px 60px 70px 80px 60px',gap:4,padding:'14px 16px',background:S.bg,fontSize:13,fontWeight:600,color:S.i3,textTransform:'uppercase',letterSpacing:'.08em'}}>
      ...
      {ch.filter(c=>!visCh||visCh.includes(c.name)).map((c,i)=>{const ci=ch.indexOf(c);const mp=getChPlan(c.name);const ps=c.planSales!=null?c.planSales:(mp.planSales||0);const barPct=ps?(Math.min((c.sales||0)/ps*100,100)):((c.sales||0)>0?100:0);const barColor=ps?((c.sales||0)>=ps?'#497B02':(c.sales||0)>=ps*0.7?'#F18B0E':'#DD2A02'):((c.sales||0)>0?S.gl:'#E4E6E9');const chCpo=(c.spent&&c.sales)?Math.round(c.spent/c.sales):c.cpo
        return<div key={i} style={{display:'grid',gridTemplateColumns:'110px 160px 70px 60px 55px 60px 70px 80px 60px',gap:4,padding:'12px 16px',borderBottom:`1px solid ${S.ln}`,alignItems:'center',fontSize:17}}>
    ...
    <div style={{background:'linear-gradient(180deg,#0F6E5E,#0B5548)',borderRadius:14,padding:'22px 16px'}}><div style={{fontSize:17,fontWeight:600,color:S.gs,marginBottom:6}}>Нужно от руководства</div><div style={{color:S.gp}}><EList items={rep.asks} canEdit={ce} onSave={v=>up({asks:v})}/></div></div>
  </></div>
}
