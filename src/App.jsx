...
function Ed({value,onSave,canEdit,multi,ph,style={}}){const[editing,setEditing]=useState(false);const[val,setVal]=useState(value);useEffect(()=>setVal(value),[value]);if(!canEdit)return<Linkify style={style}>{value||''}</Linkify>;if(!editing)return<span style={{...style,cursor:'pointer',borderBottom:'1px dashed #E3E1D8'}} onClick={()=>setEditing(true)}>{value?<Linkify style={style}>{value}</Linkify>:<span style={style}>{ph||'(кликни)'}</span>}</span>;const save=()=>{setEditing(false);if(val!==value)onSave(val)};if(multi)return<textarea value={val||''} onChange={e=>setVal(e.target.value)} onBlur={save} autoFocus rows={2} style={{width:'100%',padding:'6px 10px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:17,resize:'vertical',outline:'none',fontFamily:'inherit',fontSize:17,...style}}/>;return<input value={val||''} onChange={e=>setVal(e.target.value)} onBlur={save} onKeyDown={e=>e.key==='Enter'&&save()} autoFocus style={{width:'100%',padding:'4px 8px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:'inherit',outline:'none',...style}}/>}
function EdNum({value,onSave,canEdit,prefix,style={}}){const[editing,setEditing]=useState(false);const[val,setVal]=useState(String(value??''));useEffect(()=>setVal(String(value??'')),[value]);if(!canEdit)return<span style={style}>{prefix||''}{value!=null?(typeof value==='number'&&Math.abs(value)>=1000?value.toLocaleString():value):'—'}</span>;if(!editing)return<span style={{...style,cursor:'pointer',borderBottom:'1px dashed #E3E1D8'}} onClick={()=>setEditing(true)}>{prefix||''}{value!=null?(typeof value==='number'&&Math.abs(value)>=1000?value.toLocaleString():value):'—'}</span>;const save=()=>{setEditing(false);const n=val===''?null:Number(val);onSave(n)};return<input value={val} onChange={e=>setVal(e.target.value)} onBlur={save} onKeyDown={e=>e.key==='Enter'&&save()} autoFocus type="number" style={{width:80,padding:'2px 6px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:'inherit',outline:'none',...style}}/>}
function EdDate({value,onSave,canEdit,style={}}){const[val,setVal]=useState(value||'');useEffect(()=>setVal(value||''),[value]);if(!canEdit)return<span style={{fontSize:15,color:S.i2,...style}}>{value||'—'}</span>;return<input type="date" value={val} onChange={e=>setVal(e.target.value)} onBlur={()=>{if(val!==(value||''))onSave(val||null)}} style={{fontSize:15,padding:'2px 6px',borderRadius:10,border:`1px solid ${S.ln}`,outline:'none',color:S.i2,...style}}/>}
function EList({items,onSave,canEdit,ph,color}){const[ni,setNi]=useState('');const add=()=>{if(!ni.trim())return;onSave([...(items||[]),ni.trim()]);setNi('')};return<div><ul style={{margin:0,paddingLeft:18,fontSize:17,lineHeight:1.75,color:color||'inherit'}}>{(items||[]).map((t,i)=><li key={i} style={{display:'flex',alignItems:'flex-start',gap:6}}><Ed value={t} canEdit={canEdit} onSave={v=>{const n=[...(items||[])];n[i]=v;onSave(n)}} style={{flex:1,fontSize:17,color:'inherit'}}/>{canEdit&&<button onClick={()=>{const n=[...(items||[])];n.splice(i,1);onSave(n)}} style={{background:'none',border:'none',color:'#ccc',cursor:'pointer',fontSize:17,padding:0}}>×</button>}</li>)}</ul>{canEdit&&<div style={{display:'flex',gap:6,marginTop:6}}><input value={ni} onChange={e=>setNi(e.target.value)} placeholder={ph||'Добавить...'} onKeyDown={e=>e.key==='Enter'&&add()} style={{flex:1,padding:'4px 10px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/><button onClick={add} style={{padding:'4px 12px',borderRadius:6,border:'none',background:S.gd,color:S.gp,fontSize:15,cursor:'pointer'}}>+</button></div>}</div>}
...
function Main({profile}){
  ...
  return<div style={{minHeight:'100vh',background:S.bg}}>
    ...
    <div style={{background:'#6E9B0E',borderRadius:14,padding:'18px 22px',marginBottom:28}} className="no-print">
      ...
      {rep&&<><h1 style={{margin:'4px 0 0',fontSize:24,color:'#FAFAFA',fontWeight:400}}>Неделя <Ed value={rep.week_label} canEdit={ce} onSave={v=>upRep({week_label:v})} style={{fontSize:24,color:'#FAFAFA',fontWeight:400}}/></h1>
        ...
        <div style={{marginTop:10,display:'flex',gap:4,flexWrap:'wrap',alignItems:'center'}}>
          ...
          {isA&&rep&&<button onClick={deleteWeek} style={{padding:'8px 16px',borderRadius:8,border:'none',cursor:'pointer',fontSize:15,background:'rgba(224,75,74,0.3)',color:'#FFC1C1'}} title="Удалить текущий отчёт"><i className="ti ti-trash" style={{fontSize:18}}/></button>}
        </div>
    </div>
...
function Overview({rep,reports,projects,comments,ce,up,tTasks,tProgress,print,refreshDaily,mPlans}){
  ...
  return<div id="overview-print"><>
    ...
    <div style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,overflow:'hidden',marginBottom:28}}>
      <div style={{display:'grid',gridTemplateColumns:'110px 1fr 70px 60px 55px 60px 70px 80px 60px',gap:4,padding:'14px 16px',background:S.bg,fontSize:15,color:S.i3,fontWeight:600,textTransform:'uppercase',letterSpacing:'.06em'}}>
      ...
      {ch.filter(c=>!visCh||visCh.includes(c.name)).map((c,i)=>{const ci=ch.indexOf(c);const mp=getChPlan(c.name);const ps=c.planSales!=null?c.planSales:(mp.planSales||0);const barPct=ps?(Math.min((c.sales||0)/ps*100,100)):((c.sales||0)>0?100:0);const barColor=ps?((c.sales||0)>=ps?'#497B02':(c.sales||0)>=ps*0.7?'#F18B0E':'#DD2A02'):((c.sales||0)>0?S.gl:'#E4E6E9');const chCpo=(c.spent&&c.sales)?Math.round(c.spent/c.sales):c.cpo
        ...
        <div style={{display:'flex',alignItems:'center',gap:4}}>
          {ce&&<button onClick={()=>remCh(ci)} style={{background:'none',border:'none',color:S.ln,cursor:'pointer',fontSize:15,padding:0,lineHeight:1}}><i className="ti ti-x" style={{fontSize:15}}/></button>}
    ...
    <div style={{display:'flex',gap:12,marginBottom:12,flexWrap:'wrap'}}>
      <div style={{flex:1,background:'#F7FBEA',borderRadius:14,padding:'22px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
          <div style={{fontSize:15,color:'#7C9F55',fontWeight:600}}> Улучшилось</div>
      ...
      <div style={{flex:1,background:'#FFEEEA',borderRadius:14,padding:'22px 16px'}}>
        <div style={{fontSize:15,color:'#C77',fontWeight:600,marginBottom:6}}> Ухудшилось</div>
        <EList items={rep.worsened} canEdit={ce} onSave={v=>up({worsened:v})} color="#791F1F"/>
      </div>
    </div>
    {m.aiInsights&&<div style={{background:'#EDF5FF',borderRadius:14,padding:'22px 16px',marginBottom:12}}><div style={{fontSize:15,color:'#7B5EA7',fontWeight:600,marginBottom:4}}> Инсайты</div><Linkify style={{fontSize:17,color:'#4A3372',lineHeight:1.6}}>{m.aiInsights}</Linkify></div>}
    <div style={{display:'flex',gap:12,marginBottom:16,flexWrap:'wrap'}}>
      <div style={{flex:1,background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:'22px 16px'}}><div style={{fontSize:17,fontWeight:600,color:'#3B6D11',marginBottom:6}}> Дополнения</div><EList items={rep.manual_improved} canEdit={ce} onSave={v=>up({manual_improved:v})} color="#27500A"/></div>
      <div style={{flex:1,background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:'22px 16px'}}><div style={{fontSize:17,fontWeight:600,color:'#A32D2D',marginBottom:6}}> Дополнения</div><EList items={rep.manual_worsened} canEdit={ce} onSave={v=>up({manual_worsened:v})} color="#791F1F"/></div>
    </div>
    ...
    <div style={{background:'#6E9B0E',borderRadius:14,padding:'22px 16px'}}><div style={{fontSize:17,fontWeight:600,color:S.gs,marginBottom:6}}>Нужно от руководства</div><div style={{color:S.gp}}><EList items={rep.asks} canEdit={ce} onSave={v=>up({asks:v})}/></div></div>
  </></div>
}
...
function Tactical({tasks,progress,reports,aIdx,ce,reload,profile}){
  ...
  const TaskCard=({t})=>{const tp=getTP(t.id);const ms=t.milestones||[];const history=getAllTP(t.id);let pctVal=0;if(t.target_type==='numeric'){pctVal=t.target_value?Math.round(((tp?.current_value||0)/t.target_value)*100):0}else{const cur=tp?.milestone_status||0;pctVal=ms.length?Math.round(cur/ms.length*100):0}
    return<div style={{background:S.sf,border:`1px solid ${S.ln}`,borderRadius:14,padding:16,marginBottom:12}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><span style={{fontSize:15,fontWeight:500}}>{t.name}</span><div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:20,fontWeight:700,color:pctVal>=80?'#1D7A3F':pctVal>=50?'#BA7517':'#A32D2D'}}>{pctVal}%</span>{ce&&<button onClick={()=>toggleHide(t)} title={t.hidden?'Показать':'Скрыть'} style={{fontSize:17,background:'none',border:'none',cursor:'pointer',color:S.i3}}>{t.hidden?'':''}</button>}</div></div>
  ...
  return<>
    ...
    {showNewP&&<div style={{background:S.sf,border:`1px solid ${S.gl}`,borderRadius:14,padding:16,marginBottom:12}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'100px 1fr 120px 120px',gap:8,marginBottom:12}}>
        <input value={np.id} onChange={e=>setNp({...np,id:e.target.value})} placeholder="ID (ST01)" style={{padding:'6px 10px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <input value={np.name} onChange={e=>setNp({...np,name:e.target.value})} placeholder="Название проекта" style={{padding:'6px 10px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <input value={np.owner} onChange={e=>setNp({...np,owner:e.target.value})} placeholder="Владелец" style={{padding:'6px 10px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:15,outline:'none'}}/>
        <select value={np.priority} onChange={e=>setNp({...np,priority:e.target.value})} style={{padding:'6px 10px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:15}}><option value="key"> Ключевой</option><option value="current"> Текущий</option></select>
      </div>
      <button onClick={addProj} disabled={!np.id.trim()||!np.name.trim()} style={{padding:'10px 18px',borderRadius:8,border:'none',background:S.gd,color:S.gp,fontSize:15,fontWeight:600,cursor:'pointer'}}>Создать</button>
    </div>}
...
function CItem({c,ce,reload,onDel}){const[es,setEs]=useState(false);const[sv,setSv]=useState(c.full_text||c.summary);const saveS=async()=>{setEs(false);await supabase.from('project_comments').update({full_text:sv,summary:sv.length>120?sv.slice(0,117)+'…':sv}).eq('id',c.id);reload()};return<div style={{marginTop:6,padding:'8px 12px',background:S.bg,borderRadius:8}}><div style={{display:'flex',justifyContent:'space-between',fontSize:15,color:S.i3,marginBottom:2}}><span><b>{c.author}</b> · {c.week_start}</span><div style={{display:'flex',gap:6}}>{ce&&<button onClick={()=>setEs(!es)} style={{fontSize:15,color:S.bm,background:'none',border:'none',cursor:'pointer'}}><i className="ti ti-edit" style={{fontSize:17}}/></button>}{ce&&onDel&&<button onClick={onDel} style={{fontSize:15,color:'#ccc',background:'none',border:'none',cursor:'pointer'}}><i className="ti ti-trash" style={{fontSize:17}}/></button>}</div></div>{es?<div><textarea value={sv} onChange={e=>setSv(e.target.value)} rows={3} style={{width:'100%',padding:'6px 10px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:17,outline:'none',fontFamily:'inherit',resize:'vertical'}}/><button onClick={saveS} style={{fontSize:15,color:S.gd,background:'none',border:'none',cursor:'pointer'}}>Сохранить</button></div>:<div style={{fontSize:17,color:S.i2,whiteSpace:'pre-wrap',lineHeight:1.5}}><Linkify>{c.full_text||c.summary}</Linkify></div>}</div>}
...
function Admin({profiles,projects,reports,aIdx,reload,rep,upRep}){const[pins,setPins]=useState(rep?.pinned_projects||[]);useEffect(()=>setPins(rep?.pinned_projects||[]),[rep]);const upRole=async(id,r)=>{await supabase.from('profiles').update({role:r}).eq('id',id);reload()};const upPP=async(id,p)=>{await supabase.from('projects').update({priority:p}).eq('id',id);reload()};const togPin=async pid=>{const n=pins.includes(pid)?pins.filter(x=>x!==pid):[...pins,pid];setPins(n);if(rep)await supabase.from('weekly_reports').update({pinned_projects:n}).eq('id',rep.id);reload()}
  ...
  return<><Box title={`Каналы в Обзоре · ${rep?.week_label||''}`}><div style={{fontSize:15,color:S.i3,marginBottom:12}}>Включи/выключи каналы для отображения на этой неделе.</div>{allChNames.map(name=>{const ch=(rep?.channels||[]).find(c=>c.name===name);const on=visCh.includes(name);return<div key={name} style={{display:'flex',alignItems:'center',gap:10,padding:'6px 0',borderBottom:`1px solid ${S.ln}`,fontSize:17}}><input type="checkbox" checked={on} onChange={()=>togCh(name)} style={{cursor:'pointer'}}/><span style={{flex:1,fontWeight:500,opacity:on?1:0.4}}>{name}</span><span style={{fontSize:15,color:S.i3}}>{ch?.sales||0} продаж</span>{ch?.cpo&&<span style={{fontSize:15,color:S.i2}}>CPO ${ch.cpo}</span>}</div>})}</Box>
    <Box title="Пользователи">{profiles.map(p=><div key={p.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:`1px solid ${S.ln}`}}><div><span style={{fontWeight:500}}>{p.name||p.email}</span><span style={{fontSize:15,color:S.i3,marginLeft:8}}>{p.email}</span></div><select value={p.role} onChange={e=>upRole(p.id,e.target.value)} style={{padding:'4px 10px',borderRadius:10,border:`1px solid ${S.ln}`,fontSize:17}}><option value="admin">admin</option><option value="editor">editor</option><option value="viewer">viewer</option></select></div>)}</Box>
