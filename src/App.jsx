...
function MonthlyReport({reports,projects,comments,mPlans,setMPlans,ce,reload}){
  ...
  const savePlan=async(field,val)=>{if(!sel)return;const id=sel.key;const{data:ex}=await supabase.from('monthly_plans').select('id').eq('id',id).maybeSingle();if(!ex){const{error:ie}=await supabase.from('monthly_plans').insert({id});if(ie){alert('Ошибка создания: '+ie.message);return}};const{error}=await supabase.from('monthly_plans').update({[field]:val,updated_at:new Date().toISOString()}).eq('id',id);if(error){alert('Ошибка сохранения: '+error.message);return};const{data:fresh}=await supabase.from('monthly_plans').select('*').order('id');if(fresh)setMPlans(fresh)}
