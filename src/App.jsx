    ...
    <div style={{background:'#6E9B0E',borderRadius:14,padding:'18px 22px',marginBottom:28}} className="no-print">
      ...
      {rep&&<><h1 style={{margin:'4px 0 0',fontSize:24,color:'#FAFAFA',fontWeight:400}}>Неделя <Ed value={rep.week_label} canEdit={ce} onSave={v=>upRep({week_label:v})} style={{fontSize:24,color:'#FAFAFA',fontWeight:400}}/></h1>
        ...
        <div style={{marginTop:10,display:'flex',gap:4,flexWrap:'wrap',alignItems:'center'}}>
          ...
          {isA&&rep&&<button onClick={deleteWeek} style={{padding:'8px 16px',borderRadius:8,border:'none',cursor:'pointer',fontSize:15,background:'rgba(224,75,74,0.3)',color:'#FFC1C1'}} title="Удалить текущий отчёт"></button>}
        </div>
    </div>
...
function Overview({rep,reports,projects,comments,ce,up,tTasks,tProgress,print,refreshDaily,mPlans}){
  ...
  return<div id="overview-print"><>
    ...
    {m.aiInsights&&<div style={{background:'#EDF5FF',borderRadius:14,padding:'22px 16px',marginBottom:12}}><div style={{fontSize:15,color:'#7B5EA7',fontWeight:600,marginBottom:4}}> Инсайты от Claude</div><Linkify style={{fontSize:17,color:'#4A3372',lineHeight:1.6}}>{m.aiInsights}</Linkify></div>}
